...
- **Provider video 9-tier fallback** (lihat `.env.example`): BytePlus, xAI, LaoZhang, Evolink, Hypereal, SiliconFlow, Fal.ai, Kling, Together, PiAPI, D-ID, Agnes (async task poll, `AGNES_API_KEYS` round-robin, router-scored via `PROVIDER_CONFIG.video.agnes_video`) + Remotion; image: GeminiGen, NVIDIA, Replicate, Fal.ai, HuggingFace; prompt AI: OpenAI, Anthropic, Gemini.
...
> Last updated: 2026-08-10 — API GATE LIVE-VERIFIED (severity LOW/informational, no live exposure — full record di `docs/plans/service-layer-status.md` "Security Verification — `/api/py` Gate"): `content.aitradepulse.com` = cloudflared → `localhost:3002` (BUKAN `:8767`; stale `~/.cloudflare-router/config.yml` hanya regeneration source router-tool); dua-layer gate `src/index.ts` `/api/py/*` (rate limit 120/min/IP, body validation, server-side `X-API-Key` injection — "never trust a caller-supplied value", allowlist mirror) + `services/api.py` `enforce_api_key` (env-gated `EBOOK_API_KEY` — default-open bila unset, `secrets.compare_digest`, `PUBLIC_ALLOWLIST={"/health"}` + special-case GET `/text/articles`); probes: LIVE + LOCAL :3002 `/api/py/{health→200, pydocs→401, docs→401 (24B {"error":"Unauthorized"})}`, `/pydocs`→404, DIRECT :8767 `/docs`→401 (bukti `EBOOK_API_KEY` set di prod), `:8767` loopback-only via netstat. Remediasi documented-only (fail-closed saat key unset, allowlist-sync test, prod `docs_url=None`) — TANPA perubahan kode/config prod di pass ini (hindari scope creep ke systemd/cloudflared). Commit menyertakan: `smoke_test.py` (`_existing_telegram_id()` ambil telegram_id asli dari DB + fallback 1, `TEST_USER_ID`, param `expect_sse`), `playwright.config.ts` (testIgnore `prod-readonly.spec.ts` local-only — prod config keep spec), `tests/e2e/playwright/prod-readonly.spec.ts` baru (test `/api/py/docs` 200 lama → gated 401), plan doc (Phase 6 "Executed — wrapper migration" commit `3526537`).
