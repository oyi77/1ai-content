...
- **Infra**: PostgreSQL 15 + Redis 7 + Bull dashboard + Prometheus/Grafana (profile `monitoring`) — `docker-compose.yml`; bot TS :3000 di-manage systemd `1ai-content-bot.service` (Main PID `npm exec tsx src/index.ts`), :3002 oleh PM2 `ecosystem.config.js` (hanya app `1ai-content`, fork ×1, log `logs/pm2/`).
