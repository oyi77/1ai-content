...
while i < len(payload):
    ...
    for ch in s:
        idx = charset.find(ch)
        digits += str(idx) if idx >= 0 else ch
...
print("\n=== DOWNLOAD LINKS ===")
unique = set()
for pat in [r'href="([^"]+)"', r'src="([^"]*\.mp4[^"]*)"', r'data-url="([^"]+)"']:
    for link in re.findall(pat, decoded, re.IGNORECASE):
        if any(kw in link for kw in ['snaptik', 'rapidcdn', '.mp4', 'tiktok', 'video']):
            unique.add(link)
        elif link.endswith('.mp4') or '/download' in link or 'token=' in link:
            unique.add(link)
