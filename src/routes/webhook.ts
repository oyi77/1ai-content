...
export async function webhookRoutes(
  ...
  server.post("/webhook/telegram", async (request, reply) => {
    try {
      ...
      if (!webhookSecret && config.NODE_ENV === "production") {
        logger.error(
          "WEBHOOK_SECRET is not set in production — rejecting Telegram webhook",
        );
        return reply
          .status(503)
      ...
      bot
        .handleUpdate(
          request.body as unknown as Parameters<typeof bot.handleUpdate>[0],
        )
        .catch((err: unknown) => {
  ...
  server.post("/webhook/midtrans", async (request, reply) => {
    try {
      ...
      if (!midtransKey)
        return reply
          .status(503)
          .send({ error: "Midtrans server key not configured" });
      const expectedSig = crypto
        .createHash("sha512")
        .update(
          `${body.order_id}${body.status_code}${body.gross_amount}${midtransKey}`,
        )
        .digest("hex");
      if (!signature || !timingSafeCompare(signature, expectedSig))
      ...
      if (
        midtransFailStatuses.includes(String(body.transaction_status)) &&
        body.order_id &&
        bot
      ) {
        try {
          const tx = await prisma.transaction.findUnique({
            where: { orderId: String(body.order_id) },
          });
          if (tx?.userId) {
      ...
      sendAdminAlert("critical", "Midtrans Webhook Error", {
        error: String(error),
      });
      return reply.status(500).send({ error: "Internal server error" });
    }
  ...
  server.post("/webhook/tripay", async (request, reply) => {
    try {
      ...
      if (!tripayPrivateKey) {
        logger.error(
          "TRIPAY_PRIVATE_KEY is not set — rejecting Tripay webhook",
        );
        return reply
          .status(500)
          .send({ error: "Tripay webhook not configured" });
      }
      ...
      if (
        ...
        try {
          const tx = await prisma.transaction.findUnique({
            where: { orderId: String(body.merchant_ref) },
          });
          if (tx?.userId) {
            const dbUser = await UserService.findByTelegramId(
              BigInt(tx.userId),
            ...
            const lang = dbUser?.language || "id";
            const tKey =
              body.status === "EXPIRED" ? "payment.expired" : "payment.failed";
            await bot.telegram
              .sendMessage(
                tx.userId.toString(),
      ...
      sendAdminAlert("critical", "Tripay Webhook Error", {
        error: String(error),
      });
      return reply.status(500).send({ error: "Internal server error" });
    }
  ...
  server.post("/webhook/nowpayments", async (request, reply) => {
    try {
      ...
      if (!ipnSecret && getConfig().NODE_ENV === "production") {
        logger.error(
          "NOWPAYMENTS_IPN_SECRET is not set in production — rejecting webhook",
        );
      ...
      const result = await NowPaymentsService.handleWebhook(
        body as unknown as Parameters<
          typeof NowPaymentsService.handleWebhook
        >[0],
      );
      ...
      if (
        nowFailStatuses.includes(String(body.payment_status)) &&
        body.order_id &&
        bot
      ) {
        try {
      ...
      sendAdminAlert("critical", "NOWPayments Webhook Error", {
        error: String(error),
      });
      return reply.status(500).send({ error: "Internal server error" });
    }
  ...
  server.post("/webhook/duitku", async (request, reply) => {
    try {
      ...
      if (
        ...
        try {
          const tx = await prisma.transaction.findUnique({
            where: { orderId: String(body.merchantOrderId) },
          });
          if (tx?.userId) {
            const dbUser = await UserService.findByTelegramId(
              BigInt(tx.userId),
            ...
            await bot.telegram
              .sendMessage(
                ...
                t("payment.failed", lang, {
                  orderId: String(body.merchantOrderId),
                }),
                {
                  parse_mode: "Markdown",
                  reply_markup: {
      ...
      sendAdminAlert("critical", "Duitku Webhook Error", {
        error: String(error),
      });
      return reply.status(500).send({ error: "Internal server error" });
    }
  ...
  server.post("/webhook/1ai-payment", async (request, reply) => {
    try {
      ...
      logger.info("1ai-payment webhook received", {
        order_id: (body as Record<string, unknown>)?.order_id,
      });
      ...
      if (event.status === "failed" && event.order_id && bot) {
        try {
          const tx = await prisma.transaction.findUnique({
            where: { orderId: String(event.order_id) },
          });
          if (tx?.userId) {
            const dbUser = await UserService.findByTelegramId(
              BigInt(tx.userId),
      ...
      sendAdminAlert("critical", "1ai-payment Webhook Error", {
        error: String(error),
      });
      return reply.status(500).send({ error: "Internal server error" });
    }
