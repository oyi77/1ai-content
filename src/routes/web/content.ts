...
import { validateUrlWithDns } from "@/utils/url-validator";
...
export async function contentRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/api/storyboard", async (request, reply) => {
    try {
      ...
      const { niche, duration: durationStr, customPrompt } = body;
      if (!niche || !durationStr)
        return reply.status(400).send({ error: "Niche and duration required" });
      const duration = Number(durationStr);
      const resolvedNiche = niche;
      const nicheConfig = (NICHES as Record<string, { styles?: string[] }>)[resolvedNiche];
      ...
      return {
        ...
        caption: `🎬 ${resolvedNiche.toUpperCase()} video | ${duration}s | Generated by 1AI Content`,
        hashtags: [`#${resolvedNiche}`, "#AIVideo", "#1AIContent", "#ViralContent"],
      };
    } catch (error) {
  ...
  server.post("/api/video/create", { preHandler: [generationLimiter] }, async (request, reply) => {
    ...
    try {
      ...
      if (referenceImageUrl) {
        try { await validateUrlWithDns(referenceImageUrl); }
      ...
      const sceneData = scenes.length > 0
        ? scenes
        : [{ scene: 1, duration: Number(duration), description: customPrompt || `${resolvedNiche} marketing video` }];

      try {
        await prisma.video.create({
          data: {
            userId: user.telegramId, jobId, niche: resolvedNiche, platform,
        ...
        await enqueueVideoGeneration({
          jobId, niche: resolvedNiche, platform, duration: Number(duration),
  ...
  server.post("/api/video/analyze", async (request, reply) => {
    ...
    try {
      ...
      try { await validateUrlWithDns(videoUrl); }
      catch (urlErr) { return reply.status(400).send({ error: (urlErr as Error).message }); }
  ...
  server.post("/api/image/generate", { preHandler: [generationLimiter] }, async (request, reply) => {
    ...
    try {
      ...
      for (const url of [referenceImageUrl, avatarImageUrl]) {
        if (url) { try { await validateUrlWithDns(url); } catch (urlErr) { return reply.status(400).send({ error: (urlErr as Error).message }); } }
      }
  ...
  server.post("/api/image/describe", async (request, reply) => {
    ...
    try {
      ...
      try { await validateUrlWithDns(imageUrl); } catch (urlErr) { return reply.status(400).send({ error: (urlErr as Error).message }); }
  ...
  server.get("/video/:jobId/download", async (request, reply) => {
    try {
      ...
      if (localPath && fs.existsSync(localPath)) {
        const filename = `1ai-content-${jobId}.mp4`;
