...
export async function contentRoutes(server: FastifyInstance): Promise<void> {
    ...
    try {
      ...
      return {
        scenes: storyboard.map((s: Record<string, unknown>, i: number) => ({
          scene: i + 1,
          duration:
            (s.duration as number) || Math.floor(duration / storyboard.length),
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (referenceImageUrl) {
          try {
            await validateUrlWithDns(referenceImageUrl);
          } catch (urlErr) {
        ...
        const scenes = Array.isArray(
          (storyboard as unknown as Record<string, unknown>)?.scenes,
        )
            ...
            : [
                {
                  ...
                  description:
                    customPrompt || `${resolvedNiche} marketing video`,
                },
              ];
        ...
        try {
          await prisma.video.create({
            data: {
              ...
              storyboard: sceneData as unknown as Parameters<
                typeof prisma.video.create
              >[0]["data"]["storyboard"],
              styles: style ? [style] : [],
              generationMetadata: {
  ...
  server.post("/api/video/analyze", async (request, reply) => {
    ...
    try {
      ...
      try {
        await validateUrlWithDns(videoUrl);
      } catch (urlErr) {
        return reply.status(400).send({ error: (urlErr as Error).message });
      }
        ...
        for (const url of [referenceImageUrl, avatarImageUrl]) {
          if (url) {
            try {
              ...
              return reply
                .status(400)
                .send({ error: (urlErr as Error).message });
            }
          }
        }
  ...
  server.post("/api/image/describe", async (request, reply) => {
    ...
    try {
      ...
      try {
        await validateUrlWithDns(imageUrl);
      } catch (urlErr) {
        return reply.status(400).send({ error: (urlErr as Error).message });
      }
        ...
        try {
          ...
          if (!providerRes.ok)
            throw new ProviderError(
              "upstream",
              `Upstream fetch failed: ${providerRes.status}`,
            );
          const filename = `1ai-content-${jobId}.mp4`;
          reply.header("Content-Type", "video/mp4");
          reply.header(
