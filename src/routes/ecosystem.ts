...
const ecosystemWebhookAuth = async (
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> => {
  const key = getConfig().ECOSYSTEM_API_KEY;
  if (!key) {
    reply.status(503).send({ error: "Ecosystem API key not configured" });
    return;
  }
  const provided = (request.headers["x-api-key"] ||
    request.headers["x-ecosystem-key"]) as string | undefined;
  if (!provided || !timingSafeCompare(provided, key)) {
...
export async function ecosystemRoutes(server: FastifyInstance): Promise<void> {
   ...
   */
  server.post<{ Body: PublishResultWebhook }>(
    "/webhook/publish-result",
    { preHandler: [ecosystemWebhookAuth] },
    async (request, reply) => {
      ...
      try {
        ...
        logger.error({
          msg: "Failed to update publish status",
          error,
          contentId,
        });
      }
   ...
   */
  server.post<{ Body: ConversionWebhook }>(
    "/webhook/conversion-update",
    { preHandler: [ecosystemWebhookAuth] },
    async (request, reply) => {
        ...
        if (conversion.commission > 0) {
          ...
          if (!isNaN(telegramId) && telegramId > 0) {
            try {
              ...
              logger.warn({
                msg: "User not found for commission credit",
                userId: conversion.userId,
              });
            }
          } else {
            logger.warn({
              msg: "Non-numeric userId, skipping commission credit",
              userId: conversion.userId,
            });
          }
        }
