...
export async function agencyRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/agency/keys", async (request, reply) => {
    ...
    if (rlCount > 10) {
      return reply
        .status(429)
        .send({ error: "Too many requests. Try again in a minute." });
    }
    ...
    if (!name || typeof name !== "string" || name.length > 64) {
      return reply
        .code(400)
        .send({ error: "name must be a string up to 64 characters" });
    }
    ...
    try {
      ...
      return reply
        .status(201)
        .send({ key: rawKey, id: result.id, name: result.name });
    } catch (err) {
      if ((err as Error)?.message === "MAX_KEYS") {
        return reply
          .status(400)
          .send({ error: "Maximum 5 active API keys allowed" });
      }
