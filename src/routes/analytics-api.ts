...
export async function analyticsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/analytics/overview', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const where = userId ? { userId: BigInt(userId), createdAt: { gte: since } } : { createdAt: { gte: since } };
      ...
      const dailyTrend = await prisma.$queryRawUnsafe(`
        ...
        ORDER BY date ASC
      `, ...(userId ? [since, BigInt(userId)] : [since])) as Array<{ date: string; count: bigint; credits: number }>;
      ...
      return {
        ...
        totals: {
          ...
          completion_rate: totalVideos > 0 ? Math.round(completedVideos / totalVideos * 100) : 0,
  ...
  app.get('/api/analytics/carousels', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const carousels = await (prisma as unknown as Record<string, { count: () => Promise<number> }>).carousel?.count?.() ?? 0;
      ...
      logger.error(`[Analytics] Carousel error: ${err instanceof Error ? err.message : String(err)}`);
      return reply.status(500).send({ error: 'Failed to fetch carousel analytics' });
    }
  });
  ...
  app.get('/api/analytics/users', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      logger.error(`[Analytics] Users error: ${err instanceof Error ? err.message : String(err)}`);
