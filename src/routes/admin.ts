...
export async function adminRoutes(server: FastifyInstance): Promise<void> {
  server.addHook("onRequest", async (request, reply) => {
    ...
    const isAdminRoute =
      ...
      url.startsWith("/api/landing-config") ||
...
function registerAdminRedirects(server: FastifyInstance) {
  ...
  server.get("/admin/analytics", async (_r, reply) =>
    reply.redirect("/admin/dashboard"),
  );
  server.get("/admin/billing", async (_r, reply) =>
    reply.redirect("/admin/pricing"),
  );
  server.get("/admin/broadcast", async (_r, reply) =>
    reply.redirect("/admin/settings#broadcast"),
  );
}
...
function registerInlineApiRoutes(server: FastifyInstance) {
  ...
  server.post(
    ...
    {
      preHandler: validate({ params: idParamSchema, body: creditsBodySchema }),
    },
    async (request, reply) => {
      try {
  ...
  server.get("/api/subscriptions/active", async (request) => {
    ...
    return subs.map((s) => ({
      id: s.id,
      userId: s.userId,
      userTelegramId: s.user.telegramId.toString(),
      userUsername: s.user.username,
      userFirstName: s.user.firstName,
      userTier: s.user.tier,
      plan: s.plan,
  ...
  server.post(
    "/api/subscriptions/:id/extend",
    {
      preHandler: validate({
        params: idParamSchema,
        body: extendSubscriptionSchema,
      }),
    },
    async (request, reply) => {
      const id = BigInt((request.params as { id: string }).id);
      const days = Math.min(
        Math.max(1, ((request.body ?? {}) as { days?: number }).days || 30),
        365,
      );
      const sub = await prisma.subscription.findUnique({ where: { id } });
      if (!sub)
  ...
  server.get("/admin/*", async (request, reply) => {
    const rawPath = new URL(request.url, "http://localhost").pathname.replace(
      "/admin/",
      "",
    );
