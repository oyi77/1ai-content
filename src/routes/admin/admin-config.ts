...
const API_KEY_REGISTRY: Record<string, string> = {
  ...
  LAOZHANG_API_KEY: "LaoZhang",
  EVOLINK_API_KEY: "EvoLink",
  HYPEREAL_API_KEY: "Hypereal",
  SILICONFLOW_API_KEY: "SiliconFlow",
  FALAI_API_KEY: "Fal.ai",
  KIE_API_KEY: "Kie.ai",
  PIAPI_API_KEY: "PiAPI",
  GEMINIGEN_API_KEY: "GeminiGen",
  LINGYAAI_API_KEY: "LingyaAI",
  GETGOAPI_API_KEY: "GetGoAPI",
  APIYI_API_KEY: "APIyi",
  ZAI_API_KEY: "Z.ai",
  D_ID_API_KEY: "D-ID",
  ...
  TRIPAY_PRIVATE_KEY: "Tripay Private",
  DUITKU_MERCHANT_CODE: "DuitKu Merchant",
  DUITKU_API_KEY: "DuitKu",
...
const ALLOWED_CONFIG_CATEGORIES = [
  "provider",
  "ai_param",
  "timeout",
  "retry",
...
const configKeyParamSchema = zodToJsonSchema(
  z.object({
...
const configValueBodySchema = zodToJsonSchema(
  z.object({
    value: z.unknown(),
  }),
  "configValueBody",
...
const apiKeyNameParamSchema = zodToJsonSchema(
  z.object({
...
const apiKeyValueBodySchema = zodToJsonSchema(
  z.object({
...
export async function registerAdminConfigRoutes(
  ...
  server.put(
    ...
    async (request, reply) => {
      if (!(await verifyAdmin(request, reply))) return;
      const { category, key } = request.params as {
        category: string;
        key: string;
      };
  ...
  server.delete(
    ...
    async (request, reply) => {
      ...
      await prisma.pricingConfig.deleteMany({
        where: { category: "api_keys", key: name },
      });
      return { ok: true };
    },
  );
