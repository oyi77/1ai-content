...
export async function registerStatsRoutes(server: FastifyInstance) {
  ...
  server.get("/api/stats/overview", async () => {
    ...
    const usersByTier =
      (await prisma.$queryRaw`SELECT tier, COUNT(*)::int as count FROM users GROUP BY tier`) as {
        tier: string;
        count: number;
      }[];
    const videosByStatus =
      (await prisma.$queryRaw`SELECT status, COUNT(*)::int as count FROM videos GROUP BY status`) as {
        status: string;
        count: number;
      }[];
