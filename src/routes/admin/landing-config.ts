...
export async function registerLandingConfigRoutes(server: FastifyInstance) {
  ...
  server.get("/api/landing-config", async (_request: FastifyRequest, _reply: FastifyReply) => {
    try {
  ...
  server.post("/api/landing-config", { preHandler: validate({ body: landingConfigSchema }) }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
