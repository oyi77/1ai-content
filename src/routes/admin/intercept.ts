...
import {
  validate,
  interceptToggleSchema,
  interceptDeliverSchema,
...
export function registerInterceptRoutes(
  ...
  server.post(
    ...
    { preHandler: validate({ body: interceptToggleSchema }) },
    async (request, reply) => {
      if (!(await verifyAdmin(request, reply))) return;
      const { telegramId, enabled } = (request.body ?? {}) as {
        telegramId: string;
        enabled: boolean;
      };
      if (!telegramId)
        return reply.status(400).send({ error: "telegramId required" });
      ...
      try {
        ...
        if (error instanceof SyntaxError) {
          return reply
            .status(400)
            .send({ error: "Invalid Telegram ID format" });
        }
  ...
  server.get("/api/intercept/events/:telegramId", async (request, reply) => {
    ...
    try {
      const { InterceptService } =
        await import("../../services/intercept.service.js");
      const events = await InterceptService.getRecentEvents(
        BigInt(telegramId),
        100,
      );
      return events.map((e) => ({
  ...
  server.get("/admin/uploads/:filename", async (request, reply) => {
    ...
    const mime = ["mp4", "mov", "avi", "webm"].includes(ext)
  ...
  server.post("/api/intercept/upload", async (request, reply) => {
    ...
    try {
      const reqWithFile = request as unknown as {
        file: () => Promise<{ filename: string; file: NodeJS.ReadableStream }>;
      };
  ...
  server.post(
    "/api/intercept/deliver",
    { preHandler: validate({ body: interceptDeliverSchema }) },
    async (request, reply) => {
      if (!(await verifyAdmin(request, reply))) return;
      ...
      if (!jobId || !mediaUrl)
        return reply.status(400).send({ error: "jobId and mediaUrl required" });

      try {
        const { InterceptService } =
          await import("../../services/intercept.service.js");
        await InterceptService.deliverMedia(
          jobId,
          mediaUrl,
          mediaType || "video",
        );
