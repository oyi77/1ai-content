...
export function isNicheAllowedForPersona(
...
export function isPresetAllowedForPersona(
  persona: PersonaConfig,
  preset: string,
): boolean {
  ...
  try {
    ...
    if (dbPersonas.length > 0) {
      ...
      for (const dbP of dbPersonas) {
        const override =
          typeof dbP.value === "string"
            ? JSON.parse(dbP.value)
            : (dbP.value as Record<string, unknown>);
    ...
    logger.warn("Failed to load personas from DB, using static config", {
