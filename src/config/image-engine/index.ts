...
function indexModule(
  detection: Record<string, { keywords: string[]; label?: string; global_label?: string }>,
  moduleName: string,
  labelKey: string,
  styleKey: string,
  focusKey: string,
...
function lookupStylePrompt(module: string, styleKey: string): string {
  switch (module) {
    case 'additional_categories': return ADDITIONAL_CATEGORY_STYLES[styleKey]?.prompt_val || '';
    case 'home_decor': return INTERIOR_STYLES[styleKey]?.prompt_val || '';
    case 'fashion': return FASHION_SUB_GENRES[styleKey]?.prompt_val || '';
    case 'fnb': return FNB_STYLE_OPTIONS[styleKey]?.prompt_val || '';
    case 'skincare': return SKINCARE_STYLE_THEMES[styleKey]?.prompt_val || '';
    default: return '';
  }
}
...
function lookupEffectPrompts(module: string, effectKeys: string[]): string[] {
  ...
  for (const ek of effectKeys) {
    ...
    switch (module) {
      case 'fnb': val = FNB_EFFECT_OPTIONS[ek]?.prompt_val; break;
      case 'skincare': val = SKINCARE_TEXTURE_EFFECTS[ek]?.prompt_val; break;
      default: val = FASHION_ACCESSORY_EFFECTS[ek]?.prompt_val || ADDITIONAL_MATERIALS[ek]?.prompt_val; break;
    }
...
export function resolveImagePrompt(
  ...
  if (detected.default_style_prompt) segments.push(detected.default_style_prompt);
