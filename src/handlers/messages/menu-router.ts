 ...
 */
export async function routeMenuButton(
  ctx: BotContext,
  text: string,
): Promise<boolean> {
  ...
  const { promptsCommand, dailyCommand, trendingCommand, fingerprintCommand } =
    await import("@/commands/prompts.js");
    ...
    await ctx.reply(t("msg.image_generate_title", imgLang), {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: t("msg.img_btn_product_photo", imgLang),
          ...
          [
            {
              text: t("msg.img_btn_realestate", imgLang),
  ...
  if (menuMatch("chat")) {
    ...
    await ctx.reply(t("msg.ai_chat_active", chatLang), {
  ...
  if (menuMatch("subscription")) {
    await subscriptionCommand(ctx);
    return true;
  }
  if (menuMatch("profile")) {
    await profileCommand(ctx);
    return true;
  }
  if (menuMatch("referral")) {
    await referralCommand(ctx);
    return true;
  }
  if (menuMatch("settings")) {
    await settingsCommand(ctx);
    return true;
  }
  if (menuMatch("support")) {
    await supportCommand(ctx);
    return true;
  }
  if (menuMatch("library")) {
    await promptsCommand(ctx);
    return true;
  }
  if (menuMatch("trending")) {
...
export async function detectVideoIntent(
  ...
  if (
    !(
      ...
      /\b(buat video|create video|jadikan video|video dari|bikin video|video tentang)\b/i.test(
        text,
      )
    )
  ) {
  ...
  ctx.session.stateData = {
    ...ctx.session.stateData,
    pendingVideoPrompt: videoPrompt,
  };
  await ctx.reply(
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "🎬 Ya, buat video!",
              callback_data: "t2v_confirm_contextless",
            },
