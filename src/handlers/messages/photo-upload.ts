...
import { detectImageElements, renderElementSelectionKeyboard, buildElementSelectionMessage } from "@/handlers/callbacks/image";
 ...
 */
export async function handleCreateVideoUpload(ctx: BotContext): Promise<boolean> {
  if (ctx.session?.state !== "CREATE_VIDEO_UPLOAD" || !("photo" in ctx.message!)) {
    return false;
  }
  ...
  for (let i = 0; i < allNicheButtons.length; i += 2) {
    nicheRows.push(
      allNicheButtons.slice(i, i + 2).map((b) => ({ text: b.text, callback_data: b.callback_data })),
    );
  }

  await ctx.reply(t("msg.photo_received_niche", cvuLang), {
    reply_markup: {
      inline_keyboard: nicheRows.length > 0 ? nicheRows : [[{ text: t("msg.niche_btn_fnb", cvuLang), callback_data: "niche_fnb" }]],
    },
  });
 ...
 */
export async function handleImageReferenceWaiting(ctx: BotContext): Promise<boolean> {
  if (ctx.session?.state !== "IMAGE_REFERENCE_WAITING" || !("photo" in ctx.message!)) {
    return false;
  }
 ...
 */
export async function handleAvatarUploadWaiting(ctx: BotContext): Promise<boolean> {
  if (ctx.session?.state !== "AVATAR_UPLOAD_WAITING" || !("photo" in ctx.message!)) {
    return false;
  }
  ...
  await ctx.reply(t("msg.avatar_upload_received", lang), { parse_mode: "Markdown" });
  return true;
}
 ...
 */
export async function handleAvatarNameWaiting(ctx: BotContext): Promise<boolean> {
  if (ctx.session?.state !== "AVATAR_NAME_WAITING" || !("text" in ctx.message!)) {
    return false;
  }
  ...
  try {
    ...
    ctx.session.stateData = {};
    const lang = ctx.session?.userLang || "id";
    await ctx.reply(t("msg.avatar_created", lang, { name }), { parse_mode: "Markdown" });
  } catch (err) {
...
export async function handleAvatarTalkPhoto(ctx: BotContext): Promise<boolean> {
  if (ctx.session?.state !== "avatar_talk_photo" || !("photo" in ctx.message!)) {
    return false;
  }
