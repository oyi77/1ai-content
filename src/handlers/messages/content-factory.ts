 ...
 */
export async function handleVoiceTextWaiting(
  ctx: BotContext,
): Promise<boolean> {
  ...
  try {
    const result = await contentFactoryService.synthesizeSpeech(text, {
      language,
      voice,
    });
    ...
    logger.error(
      `[Voice Text] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
    await ctx.reply(
 ...
 */
export async function handleLoopAudioWaiting(
  ctx: BotContext,
): Promise<boolean> {
  ...
  if (!fileId) {
    await ctx.reply(
      "⚠️ Kirim file audio (.mp3/.wav) untuk dijadikan looping video.",
    );
    return true;
  }

  const visualType =
    (ctx.session.stateData?.visualType as string) || "gradient";
  ctx.session.state = "DASHBOARD";
  ctx.session.stateData = {};

  await ctx.reply(
    `🔁 Downloading audio & creating loop...\n🎨 Visual: ${visualType}\n⏳ Proses ini butuh beberapa menit...`,
  );
  ...
  try {
    ...
    logger.error(
      `[Loop Audio] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
