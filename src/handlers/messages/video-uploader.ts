...
export async function handleVideoElementPrecheck(
  ...
  try {
    const photoUrl = (
      await ctx.telegram.getFileLink(uploadedPhotos[0].fileId)
    ).toString();
    ...
    if (analysis.success && analysis.prompt) {
      ...
      // Always show element selection when analysis succeeds (not just when both types detected)
      {
        ...
        await ctx.reply(
          buildElementSelectionMessage(
            elements,
            elements.characterDesc,
            elements.productDesc,
          ),
          {
            parse_mode: "Markdown",
    ...
    logger.warn(
      "Video element precheck failed (non-fatal), proceeding directly",
    );
  }
...
export async function handleVideoCreationImage(
  ...
  if (photos.length === 0) {
    await ctx.reply("Belum ada foto. Kirim gambar referensi atau /skip.");
    return;
  }
  ...
  try {
    ...
    try {
      ...
      logger.error(
        "wget failed downloading reference image:",
        (wgetErr as Error).message,
      );
      throw new ProviderError(
        "video-uploader",
        `Failed to download reference image: ${(wgetErr as Error).message}`,
      );
    }
    ...
    try {
      ...
      await ctx.reply(
        t("uploader.analyzing_photos", uploaderLang, { count: photos.length }),
        {
          parse_mode: "Markdown",
        },
      );
    ...
    if (visionInsights && storyboard && storyboard.length > 0) {
      ...
      if (videoSel && videoAnalysis) {
        ...
        if (videoSel.keepProduct && !videoSel.keepCharacter) {
          ...
          parts.push(
            `Product to feature: ${pDesc}. Person/character: ${cDesc}.`,
          );
        }
        ...
        if (
          !videoSel.keepProduct &&
          !videoSel.keepCharacter &&
          !videoSel.keepBackground
        ) {
    ...
    const userMessage = actionableError(
