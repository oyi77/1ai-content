...
export async function handleMediaIntentCallback(
  ctx: BotContext,
  data: string,
): Promise<boolean> {
  if (!data.startsWith("media_intent_") && data !== "t2v_confirm_contextless")
    return false;

  await ctx.answerCbQuery().catch(() => {});

  const lang = ctx.session?.userLang || "id";
  const pendingUrl = (ctx.session.stateData as Record<string, unknown>)
    ?.pendingMediaUrl as string | undefined;
  const pendingUrls = (ctx.session.stateData as Record<string, unknown>)
    ?.pendingMediaUrls as string[] | undefined;
  const pendingPrompt = (ctx.session.stateData as Record<string, unknown>)
    ?.pendingPrompt as string | undefined;
  ...
  if (data === "media_intent_i2i") {
    ...
    await ctx.editMessageText("🔍 _Menganalisis gambar..._", {
      parse_mode: "Markdown",
    });
  ...
  if (data === "media_intent_i2v" || data === "media_intent_batch_i2v") {
    ...
    await ctx.editMessageText("🔍 _Menganalisis gambar untuk video..._", {
      parse_mode: "Markdown",
    });
    ...
    (ctx.session.videoCreation as Record<string, unknown>).pendingPhotos =
      photos.map((url: string) => ({ fileId: "", url }));
    ...
    try {
      ...
      if (analysis.success && analysis.prompt) {
        ...
        if (elements.hasProduct)
          detected.push(`📦 Produk: _${elements.productDesc || "terdeteksi"}_`);
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  ...
                  {
                    text: "🎬 Jadikan Video",
                    callback_data: "media_intent_i2v",
                  },
                ],
        ...
        ctx.session.stateData = {
          pendingMediaUrl: pendingUrl,
          pendingMediaType: "photo",
        };
      } else {
        await ctx.editMessageText(
          "❌ Tidak bisa menganalisis gambar. Coba lagi.",
        );
      }
    } catch (err) {
  ...
  if (data === "media_intent_v2v") {
    ...
    await ctx.editMessageText("🔍 _Menganalisis video..._", {
      parse_mode: "Markdown",
    });
    ctx.session.state = "DASHBOARD";
    ctx.session.stateData = {};
    ...
    void (async () => {
      try {
        ...
        if (result.success && result.prompt) {
          ...
          await telegram.sendMessage(
            chatId,
            "❌ Tidak bisa menganalisis video. Coba lagi.",
          );
        }
      } catch (err) {
  ...
  if (data === "media_intent_v2t") {
    ...
    await ctx.editMessageText(
      "🔍 _Menganalisis video... (bisa sampai 30 detik)_",
      { parse_mode: "Markdown" },
    );
    ctx.session.state = "DASHBOARD";
    ctx.session.stateData = {};
  ...
  if (data === "t2v_confirm_contextless") {
    ...
    const videoPrompt = ctx.session.stateData?.pendingVideoPrompt as
      | string
      | undefined;
    if (!videoPrompt) {
    ...
    if (!user || Number(user.creditBalance) < creditCost) {
      await ctx.editMessageText(
        "❌ Kredit tidak cukup untuk membuat video. Top up terlebih dahulu.",
        {
          reply_markup: {
            inline_keyboard: [
    ...
    await ctx.editMessageText("⏳ _Menambahkan ke antrian video..._", {
      parse_mode: "Markdown",
    });
    ...
    try {
      ...
      await telegram.sendMessage(
        chatId,
        "❌ Gagal menambah ke antrian. Coba lagi.",
      );
    }
    return true;
  }
