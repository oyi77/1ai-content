 ...
 */
export async function handleContentFactoryCallbacks(
  ctx: BotContext,
  data: string,
): Promise<boolean> {
  try {
    ...
    if (data.startsWith("suno_preset_")) {
      const presetMap: Record<string, { prompt: string; style: string }> = {
        lofi: {
          prompt: "lo-fi hip hop chill beats, relaxing study music",
          style: "lofi",
        },
        piano: {
          prompt: "romantic piano instrumental, soft and emotional",
          style: "classical",
        },
        corporate: {
          prompt:
            "upbeat corporate background music, positive and motivational",
          style: "corporate",
        },
        cinematic: {
          prompt: "epic cinematic trailer music, dramatic orchestral",
          style: "cinematic",
        },
        meditation: {
          prompt: "calming meditation music, zen ambient sounds",
          style: "ambient",
        },
      ...
      try {
        const result = await contentFactoryService.generateSunoMusic(
          preset.prompt,
          {
            style: preset.style,
            instrumental: true,
        ...
        logger.error(
          `[Suno Callback] Error: ${err instanceof Error ? err.message : String(err)}`,
        );
    ...
    if (data.startsWith("voice_select_")) {
      const voiceMap: Record<
        string,
        { voice: string; lang: string; label: string }
      > = {
        id_male: {
          voice: "id-ID-ArdiNeural",
          lang: "id",
          label: "Indonesia Male",
        },
        id_female: {
          voice: "id-ID-GadisNeural",
          lang: "id",
          label: "Indonesia Female",
        },
        en_male: {
    ...
    if (data.startsWith("music_preset_")) {
      ...
      await ctx.answerCbQuery("🎶 Generating...");
      await ctx.reply(`🎶 Generating: *${preset}*...`, {
        parse_mode: "Markdown",
      });

      try {
        const result = await contentFactoryService.generateMusic(preset, {
          duration: 60,
        });
        ...
        logger.error(
          `[Music Callback] Error: ${err instanceof Error ? err.message : String(err)}`,
        );
        await ctx.reply("❌ Error generating music.");
      }
      return true;
    ...
    if (data.startsWith("loop_from_")) {
      ...
      try {
        ...
        if (result.success && result.video_path) {
          ...
          await ctx.reply(
            `❌ ${result.error || "Gagal membuat looping video"}`,
          );
        }
      } catch (err: unknown) {
        logger.error(
          `[Loop Callback] Error: ${err instanceof Error ? err.message : String(err)}`,
        );
    ...
    logger.error(
      `[ContentFactory Callback] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
    return false;
  }
}
