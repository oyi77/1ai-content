...
export async function handleNavigationCallbacks(
  ...
  if (data === "chat_ai" || data === "open_chat") {
    ...
    await ctx
      .editMessageText(t("cb2.ai_assistant_active", lang), {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [
            [
      ...
      .catch(async () => {
        await ctx.reply(t("cb2.ai_assistant_fallback", lang), {
          parse_mode: "Markdown",
        });
      });
    return true;
  ...
  if (data === "contact_support") {
    await ctx.answerCbQuery();
    const lang = ctx.session?.userLang || "id";
    await ctx.reply(t("cb2.contact_support", lang), {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [{ text: t("btn.main_menu", lang), callback_data: "main_menu" }],
