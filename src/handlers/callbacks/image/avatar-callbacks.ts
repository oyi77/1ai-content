...
export async function handleAvatarView(ctx: BotContext, avatarId: number): Promise<boolean> {
...
export async function handleAvatarSetDefault(ctx: BotContext, avatarId: number): Promise<boolean> {
  const telegramId = BigInt(ctx.from!.id);
  const lang = ctx.session?.userLang || 'id';
...
export async function handleAvatarDelete(ctx: BotContext, avatarId: number): Promise<boolean> {
  const telegramId = BigInt(ctx.from!.id);
  const lang = ctx.session?.userLang || 'id';
