...
import { handleAvatarManage, handleAvatarAdd, handleAvatarView, handleAvatarSetDefault, handleAvatarDelete } from './avatar-callbacks';
...
export async function handleImageCallbacks(ctx: BotContext, data: string): Promise<boolean> {
    ...
    await ctx.editMessageText(t('cb.image_generate_title', lang), {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: t('btn.real_estate', lang), callback_data: 'img_realestate' }],
  ...
  if (data === 'image_from_prompt') {
    ...
    const nicheToCategory: Record<string, string> = {
      fnb: 'fnb', food: 'fnb', fashion: 'product',
      health: 'product', tech: 'product', finance: 'product',
      education: 'product', entertainment: 'product', travel: 'realestate',
    };
    const sessionNiche = (ctx.session?.selectedNiche ||
      ctx.session?.stateData?.addingPromptNiche || 'product') as string;
...
async function handleImgRefAvatar(ctx: BotContext, avatarId: number): Promise<boolean> {
  ...
  await ctx.editMessageText(t('cb.using_avatar', lang2, { name: avatar.name }), {
    parse_mode: 'Markdown',
...
async function handleElementSelection(ctx: BotContext, data: string): Promise<boolean> {
  const { renderElementSelectionKeyboard, buildElementSelectionMessage } = await import('./element-ui.js');
  ...
  if (data === 'imgelem_product' || data === 'imgelem_character' || data === 'imgelem_background') {
    ...
    if (ctx.session.state === 'VIDEO_ELEMENT_SELECTION') {
      ...
      const videoAnalysis = ctx.session.videoCreation?.videoAnalysisResult as
        { hasCharacter: boolean; hasProduct: boolean; characterDesc?: string; productDesc?: string } | undefined;
      await ctx.editMessageText(
    ...
    await ctx.editMessageText(
      ...
      { parse_mode: 'Markdown', reply_markup: renderElementSelectionKeyboard(sel) },
    );
    return true;
  }
  ...
  if (data === 'imgelem_confirm') {
    ...
    if (pendingPrompt) {
      ...
      const elementAnalysis = ctx.session.stateData?.imageAnalysisResult as
        { productDesc: string; characterDesc: string; backgroundDesc: string } | undefined;
      ctx.session.state = 'DASHBOARD';
    ...
    await ctx.editMessageText(t('cb.describe_image', lang, { hint }) + textOnlyNote, {
      parse_mode: 'Markdown',
      reply_markup: { inline_keyboard: [[{ text: t('btn.back', lang), callback_data: 'image_generate' }]] },
    });
    return true;
