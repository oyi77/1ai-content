/**
 * Image handlers — Main callback router
 *
 * Dispatches image-related callbacks to focused sub-handlers.
 */
import { t } from '@/i18n/translations';
import { AvatarService } from '@/services/avatar.service';
import { buildElementSelectionMessage, renderElementSelectionKeyboard, btnBackMain, categoryNames } from './element-ui';
import { handleImageGeneration } from './handle-generation';
import { handleAvatarCallbacks } from './avatar-callbacks';
...
/**
 * Main image callback router.
 * Returns true if the callback was handled, false to fall through.
 */
export async function handleImageCallbacks(ctx: BotContext, data: string): Promise<boolean> {
  ...
  if (
    data === 'avatar_manage' ||
    data === 'avatar_add' ||
    data.startsWith('avatar_view_') ||
    data.startsWith('avatar_default_') ||
    data.startsWith('avatar_delete_') ||
    data.startsWith('imgref_avatar_')
  ) {
    return handleAvatarCallbacks(ctx, data);
  }
  ...
  if (data === 'imgref_skip') {
    ...
    await ctx.editMessageText(t('cb.describe_image', lang, { hint }), {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [[{ text: t('btn.back', lang), callback_data: 'image_generate' }]],
      },
    });
    return true;
  }
  ...
  if (data === 'imgelem_product' || data === 'imgelem_character' || data === 'imgelem_background') {
    ...
    if (ctx.session.state === 'VIDEO_ELEMENT_SELECTION') {
      ...
      const videoAnalysis = ctx.session.videoCreation?.videoAnalysisResult as
        | { hasCharacter: boolean; hasProduct: boolean; characterDesc?: string; productDesc?: string }
        | undefined;
  ...
  if (data === 'imgelem_confirm') {
    ...
    if (pendingPrompt) {
      ...
      await executeImageGeneration(ctx, pendingPrompt, {
        category, referenceImageUrl, mode, elementSelection: sel, elementAnalysis,
      });
      return true;
    }
