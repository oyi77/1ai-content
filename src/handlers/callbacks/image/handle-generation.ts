...
export async function handleImageGeneration(ctx: BotContext, category: string) {
          ...
          try {
            await telegramClient.sendPhoto(chatId, result.imageUrl, {
              caption: t('cb2.image_success', lang, {
                prompt: existingClonePrompt.slice(0, 100) + (existingClonePrompt.length > 100 ? '...' : ''),
              }),
              parse_mode: 'Markdown',
            });
          } catch (sendErr) {
  ...
  await ctx.editMessageText(
    t('cb.image_gen_header', lang, { category: categoryNames[category], cost: String(creditCost) }),
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: t('cb.btn_upload_ref', lang), callback_data: 'imgref_upload' }],
          ...(avatarButtons.length > 0 ? [avatarButtons] : []),
          [{ text: t('cb.btn_describe_only', lang), callback_data: 'imgref_skip' }],
          [btnBackMain(lang)],
        ],
      },
    },
