...
export async function handleLegacyCreationCallback(
  ctx: BotContext,
  data: string,
): Promise<boolean> {
  ...
  if (data === "vcreate_confirm") {
    ...
    if (!dbUser || Number(dbUser.creditBalance) < template.creditCost) {
      ...
      await ctx.editMessageText(
        t("creation.insufficient_credits", lang, {
          cost: template.creditCost,
          balance: Number(dbUser?.creditBalance || 0),
        }),
        {
          parse_mode: "Markdown",
          reply_markup: {
    ...
    try {
      ...
      generateVideoWithFallback({
        ...
        .then(async (result) => {
          if (result.success && result.videoUrl) {
            ...
            await ctx.telegram.sendVideo(ctx.chat!.id, result.videoUrl, {
              caption: t("creation.video_done", vLang, {
                name: template.name,
                scenes: template.sceneCount,
                duration,
              }),
                  ...
                  [
                    {
                      ...
                      callback_data: `publish_video_${actualJobId}`,
                    },
                  ],
                  [
                    {
                      text: "👍 Good",
                      callback_data: `feedback_good_${actualJobId}`,
                    },
                    {
                      text: "👎 Needs Work",
                      callback_data: `feedback_bad_${actualJobId}`,
                    },
                  ],
                  [
                    {
        ...
        .catch(async (error) => {
          ...
          await ctx.telegram.sendMessage(
            ctx.chat!.id,
            t("creation.credits_refunded", rfLang, {
              jobId: actualJobId,
        ...
        }),
        {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {
                  text: t("videos.btn_my_videos", gLang),
      ...
      await ctx.reply(
        t("creation.generation_failed_refund", efLang, {
          error: (error as Error).message,
        }),
        { parse_mode: "Markdown" },
      );
  ...
  if (data === "create_image_new") {
    ...
    await ctx.editMessageText(t("creation.image_generate_title", ciLang), {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: t("creation.btn_product_photo", ciLang),
          ...
          [
            {
              text: t("creation.btn_realestate", ciLang),
          ...
          [
            {
              text: t("creation.btn_avatar", ciLang),
