...
export function setupHandlers(bot: Telegraf<BotContext>): void {
  ...
  bot.on("pre_checkout_query", async (ctx) => {
    try {
      ...
      const valid =
        !!match &&
        STARS_PACKAGES.some((p) => p.credits === parseInt(match[1], 10));
      await ctx.answerPreCheckoutQuery(
        valid,
        valid ? undefined : "Invalid payment payload",
      );
    } catch (error) {
  ...
  bot.on("successful_payment", async (ctx) => {
    try {
      ...
      if (!pkg) {
        logger.error(
          `Stars payment: invalid credits ${credits} in payload ${payload}`,
        );
        return;
      }
      ...
      try {
        ...
        if ((e as { code: string })?.code === "P2002") {
          ...
          const isDuplicate =
            target.includes("order_id") ||
            target.includes("gateway_transaction_id");
          if (!isDuplicate) throw e;
          logger.warn(
            `Stars duplicate payment ignored: ${payment.telegram_payment_charge_id}`,
          );
      ...
      logger.info(
        `Stars payment success: ${credits} credits for user ${userId}`,
      );
