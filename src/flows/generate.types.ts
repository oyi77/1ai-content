...
export function clearGenerateSession(ctx: BotContext): void {
  ...
  const fields = [
    ...
    "generateAspectRatio",
    "generateResolution",
    "generateCampaignSize",
    "generateScenes",
    "generateStoryboardMode",
    "generateManualStoryboard",
    "generateManualTranscript",
    "customPresetConfig",
    "generatePhotos",
    "generatePhotoCount",
  ] as const;
  for (const f of fields)
    delete (ctx.session as unknown as Record<string, unknown>)[f];
