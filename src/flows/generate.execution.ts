...
export async function executeGeneration(ctx: BotContext): Promise<void> {
  ...
  try {
    ...
    if (!user) {
      await ctx.reply(t("gen.user_not_found", "id"));
      return;
    }
    ...
    if (action === "image_set")
      ...
      cost = await CampaignService.getCampaignCost(
        (session.generateCampaignSize as 5 | 10) || 5,
      );
    ...
    if (useFreeSlot) {
      ...
      if (template) {
        try {
          ...
          await ctx.reply(
            t("gen.free_trial_video", lang, { niche: userNiche }) +
      ...
      await ctx.reply(t("gen.generating_trial", lang), {
        parse_mode: "Markdown",
      });
    ...
    if (action === "image_set") {
      ...
      const selectedRes = (session.generateResolution || "standard") as
        | "standard"
        | "hd"
        | "ultra";
      const imgParams = {
        ...
        mode: (photoUrl ? "img2img" : "text2img") as
          | "img2img"
          | "text2img"
          | "ip_adapter",
      };
      ...
      try {
        ...
        await ctx.reply(
          t("gen.imgset_preview_offer", lang, {
          ...
          }),
          {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: t("gen.btn_preview_images", lang),
                    callback_data: `imgset_preview_${video.jobId}`,
                  },
                ],
                [
                  {
                    text: t("gen.btn_skip_preview", lang),
        ...
        generateVideoAsync(
          ...
          await ctx.telegram
            .sendMessage(ctx.chat!.id, t("gen.video_failed_refund", lang))
            .catch(() => {});
        });
    ...
    if (action === "video") {
      ...
      const useManualStoryboard =
        session.generateStoryboardMode === "manual" &&
        session.generateManualStoryboard?.length;
      ...
      try {
        ...
        try {
          ...
          generateVideoAsync(
            ...
            await ctx.telegram
              .sendMessage(ctx.chat!.id, t("gen.campaign_failed", lang))
              .catch(() => {});
          });
          await ctx.reply(t("gen.video_processing", lang));
        }
        ...
        await ctx.reply(t("gen.campaign_failed", lang), {
          parse_mode: "Markdown",
        });
        clearGenerateSession(ctx);
        return;
