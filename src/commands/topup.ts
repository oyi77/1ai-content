  ...
  try {
    ...
    const upsellRow = !subscribed
      ? [
          [
            {
              text: "💡 Subscribe for Cheaper Rates!",
      ...
      [
        {
          ...
          callback_data: "topup_stars_menu",
        },
      ],
      [
        {
          text: "💎 Crypto (USDT/BNB/MATIC/TON)",
          callback_data: "topup_crypto_menu",
        },
      ],
    ];
 ...
 */
export async function handleTopupSelection(
  ctx: BotContext,
  packageId: string,
): Promise<void> {
  try {
    ...
    try {
      ...
      try {
        await ctx.reply(t("topup.create_failed", lang));
      } catch {
        /* ignore */
      }
    }
 ...
 */
export async function handlePaymentGateway(
  ctx: BotContext,
  packageId: string,
  gateway: string,
): Promise<void> {
  try {
    ...
    const gatewayDisplayName = gateway === "tripay" ? "Tripay" : "Midtrans";
    const transaction = await PaymentService.createTransaction({
      userId: BigInt(user.id),
      packageId,
      username: user.username || user.first_name,
      gateway: normalizedGateway as
        | "midtrans"
        | "tripay"
        | "duitku"
        | "nowpayments",
    });
    const paymentUrl = (transaction.redirectUrl as string) || "";

    await ctx.editMessageText(
      t("topup.payment_ready", lang, {
    ...
    try {
      ...
      try {
        await ctx.reply(t("topup.create_failed", "id"));
      } catch {
        /* ignore */
      }
    }
 ...
 */
export async function showDuitkuPaymentMethods(
  ctx: BotContext,
  packageId: string,
): Promise<void> {
  try {
 ...
 */
export async function handleDuitkuMethodSelection(
  ctx: BotContext,
  packageId: string,
...
export async function checkPayment(
  ...
  try {
    ...
    if (transaction.status === "success") {
      await ctx.editMessageText(
        t("topup.success", lang, {
          credits: Number(transaction.creditsAmount),
        }),
        { parse_mode: "Markdown" },
      );
      ...
      await ctx.editMessageText(
        t("topup.failed_status", lang, { status: transaction.status }),
        { parse_mode: "Markdown" },
      );
    }
  } catch (error) {
...
export async function handleTopupExtraCredit(
  ctx: BotContext,
  credits: number,
): Promise<void> {
  try {
    ...
    if (!dbUser) {
      await ctx.editMessageText(
        t("error.user_not_found", ctx.from?.language_code || "id"),
      );
      return;
    }
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [
              {
                text: t("topup.btn_paid", lang),
                callback_data: `check_payment_${gatewayRes.orderId}`,
              },
            ],
            [{ text: t("btn.back", lang), callback_data: "topup" }],
          ],
...
export async function handleStarsMenu(ctx: BotContext): Promise<void> {
  try {
    ...
    const starsList = STARS_PACKAGES.map(
      (pkg) =>
...
export async function handleStarsInvoice(
  ...
  try {
    ...
    await ctx.telegram.sendInvoice(ctx.chat!.id, {
      ...
      description: t("topup.stars_invoice_desc", sLang, {
        credits: pkg.credits,
      }),
      ...
      prices: [
        {
          label: t("topup.stars_invoice_label", sLang, {
            credits: pkg.credits,
          }),
          amount: pkg.stars,
        },
...
export async function handleCryptoMenu(ctx: BotContext): Promise<void> {
  try {
    ...
    const cryptoList = CRYPTO_PACKAGES.map(
      (pkg) =>
 ...
 */
export async function handleCryptoCoinSelect(
  ctx: BotContext,
  credits: number,
): Promise<void> {
  try {
    ...
    buttons.push([
      { text: t("btn.back", csLang), callback_data: "topup_crypto_menu" },
    ]);

    await ctx.editMessageText(
      t("topup.crypto_coin_select", csLang, {
        credits,
...
export async function handleCryptoPayment(
  ...
  try {
    ...
    try {
      await ctx.editMessageText(
        t("topup.crypto_payment_failed", await getLang(ctx)),
      );
    } catch {
      try {
        await ctx.reply(t("topup.crypto_payment_failed", "id"));
      } catch {
        /* ignore */
      }
    }
