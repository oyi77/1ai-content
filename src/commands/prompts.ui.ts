...
import { PROMPT_LIBRARY, TRENDING_PROMPTS, findAnyPrompt, getPromptById } from "./prompts.data";
...
export async function showNichePrompts(ctx: BotContext, nicheKey: string, edit = false): Promise<void> {
  ...
  if (telegramId) {
    try {
      ...
      if (dbUser) savedPrompts = await SavedPromptService.getByUser(dbUser.id as unknown as bigint, nicheKey);
    } catch { /* non-critical */ }
  }
  ...
  try {
    if (edit) await ctx.editMessageText(msg, { parse_mode: "Markdown", reply_markup: markup });
    else await ctx.reply(msg, { parse_mode: "Markdown", reply_markup: markup });
  } catch { await ctx.reply(msg, { parse_mode: "Markdown", reply_markup: markup }); }
}
...
export async function showPromptDetail(ctx: BotContext, promptId: string, edit = false): Promise<void> {
  ...
  const markup = {
    inline_keyboard: [
      ...
      [{ text: "🔧 Customize", callback_data: `customize_prompt_${promptId}` }, { text: "💾 Simpan", callback_data: `save_prompt_${promptId}` }],
      [{ text: "◀️ Kembali ke Niche", callback_data: `prompts_niche_${p.niche}` }],
    ],
  };
  ...
  if (ctx.session) {
    ...
    ctx.session.stateData = { ...(ctx.session.stateData || {}), selectedPrompt: p.prompt, selectedPromptId: p.id };
  }
}
...
export async function showCustomizePrompt(ctx: BotContext, promptId: string, edit = false): Promise<void> {
  ...
  const markup = {
    inline_keyboard: [
      [{ text: "🎬 Cinematic", callback_data: `cust_style_cinematic_${promptId}` }, { text: "⚡ Dramatic", callback_data: `cust_style_dramatic_${promptId}` }, { text: "✨ Minimalist", callback_data: `cust_style_minimal_${promptId}` }],
      [{ text: "🌅 Golden Hour", callback_data: `cust_light_golden_${promptId}` }, { text: "💡 Studio", callback_data: `cust_light_studio_${promptId}` }, { text: "🌙 Moody", callback_data: `cust_light_moody_${promptId}` }],
...
export async function showMyPrompts(ctx: BotContext, nicheKey: string, edit = false): Promise<void> {
  try {
    ...
    if (saved.length === 0) {
      ...
      const markup = { inline_keyboard: [[{ text: `◀️ Kembali ke ${niche?.emoji} ${niche?.label}`, callback_data: `prompts_${nicheKey}` }]] };
      if (edit) await ctx.editMessageText(msg, { parse_mode: "Markdown", reply_markup: markup }).catch(() => ctx.reply(msg, { parse_mode: "Markdown", reply_markup: markup }));
      else await ctx.reply(msg, { parse_mode: "Markdown", reply_markup: markup });
      return;
    }
