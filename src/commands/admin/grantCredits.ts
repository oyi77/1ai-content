...
export async function adminGrantCreditsCommand(ctx: BotContext): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `Admin ${userId} granted ${amount} credits to user ${targetUserId}. Reason: ${reason}`,
    );
 ...
 */
export async function adminDeductCreditsCommand(
  ctx: BotContext,
): Promise<void> {
  const userId = ctx.from?.id;
  ...
  try {
    ...
    logger.info(
      `Admin ${userId} deducted ${amount} credits from user ${targetUserId}. Reason: ${reason}`,
    );
    ...
    await ctx.reply(
      `❌ Failed to deduct credits: ${(error as Error).message || "Unknown"}`,
    );
  }
}
