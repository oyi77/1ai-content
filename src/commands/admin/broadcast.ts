...
export async function adminBroadcastCommand(ctx: BotContext): Promise<void> {
  ...
  while (i < args.length) {
    ...
    if (arg.startsWith("--")) {
      ...
      const filterName = rawName.replace(/-([a-z])/g, (_, c) =>
        c.toUpperCase(),
  ...
  try {
    logger.info(
      `Admin ${userId} broadcasting message: ${broadcastMessage} with filters:`,
      filters,
    );
    ...
    if (filters.tier || filters.region || filters.activeSince) {
      ...
      if (filters.activeSince) {
        ...
        if (match) {
          ...
          logger.warn(
            `Invalid activeSince format: ${filters.activeSince}, ignoring`,
          );
        }
      }
