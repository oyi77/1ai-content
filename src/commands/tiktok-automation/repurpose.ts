...
export async function repurposeCommand(ctx: BotContext): Promise<void> {
  ...
  for (let i = 0; i < parts.length; i++) {
    const p = parts[i];
    if (p === "--duration" && parts[i + 1]) {
      opts.duration = parseInt(parts[i + 1], 10) || 180;
      i++;
    } else if (p === "--platform" && parts[i + 1]) {
      opts.platform = parts[i + 1];
      i++;
    } else if (p === "--niche" && parts[i + 1]) {
      opts.niche = parts[i + 1];
      i++;
    } else if (p === "--style" && parts[i + 1]) {
      opts.style = parts[i + 1];
      i++;
    } else if (p === "--color" && parts[i + 1]) {
      opts.color = parts[i + 1];
      i++;
    } else if (p === "--transition" && parts[i + 1]) {
      opts.transition = parts[i + 1];
      i++;
    } else if (p === "--overlay" && parts[i + 1]) {
      opts.overlay = parts[i + 1];
      i++;
    } else if (p === "--watermark" && parts[i + 1]) {
      opts.watermark = parts[i + 1];
      i++;
    } else if (p === "--bgm" && parts[i + 1]) {
      opts.bgm = parts[i + 1];
      i++;
    } else if (p === "--subtitles") {
      opts.subtitles = true;
  ...
  if (urls.length < 2) {
    await ctx.reply(
      "❌ Minimal 2 URL diperlukan.\n\nContoh: `/repurpose <url1> <url2>`",
      { parse_mode: "Markdown" },
    );
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      const segments = result.segments_used as
      ...
      await ctx.reply(
        `❌ Gagal repurpose: ${String(result.error ?? "Unknown error")}`,
      );
    }
  } catch (err: unknown) {
    logger.error(
      `[Repurpose] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
