...
export async function handleTikTokAutomationCallbacks(
  ctx: BotContext,
  data: string,
): Promise<boolean> {
  const user = ctx.from;
  ...
  try {
    ...
    if (data.startsWith("carousel_style_")) {
      ...
      const topic = String(
        ctx.session.stateData?.calendarTopic ??
          ctx.session.stateData?.carouselTopic ??
          "",
      );
      if (!topic) {
        ctx.session.stateData = {
    ...
    if (data.startsWith("ab_type_")) {
      ...
      if (!topic) {
        await ctx.answerCbQuery();
        await ctx.reply(
          "❌ Topik tidak ditemukan. Gunakan `/abtest create <topic>`",
          { parse_mode: "Markdown" },
        );
        return true;
      }
      ...
      const variantA =
        typeof test.variant_a === "object" && test.variant_a !== null
          ? JSON.stringify(test.variant_a).slice(0, 200)
          : String(test.variant_a ?? "");
      const variantB =
        typeof test.variant_b === "object" && test.variant_b !== null
    ...
    if (data.startsWith("ab_start_")) {
      ...
      if (test) {
        await ctx.reply(
          `✅ Test "${test.name}" started! Publish both variants and track metrics.`,
        );
      }
      return true;
    }
    ...
    if (data === "cal_list_all") {
      ...
      if (calEntries.length === 0) {
        await ctx.reply(
          "📅 Belum ada jadwal konten.\n\nKetik `/calendar schedule <topic>` untuk mulai.",
          { parse_mode: "Markdown" },
        );
      } else {
          ...
          const status =
            e.status === "published"
              ...
              : e.status === "scheduled"
                ...
                : "📝";
          lines.push(
            `${status} ${emoji} ${e.topic.slice(0, 35)} — ${e.scheduled_at}`,
          );
        }
        if (calEntries.length > 10)
    ...
    if (data === "ab_list_completed") {
      ...
      if (abDone.length === 0) {
        ...
        for (const t of abDone) {
          const winner =
            t.winner === "tie" ? "🤝 Tie" : `🏆 Variant ${t.winner}`;
    ...
    if (data === "autopilot_detailed") {
      ...
      if (apStatus.jobs?.length > 0) {
        ...
        for (const job of apStatus.jobs) {
          lines.push(
            `• ${job.name} — ${job.status} (${job.config.content_type}) — ${job.config.posting_times.join(", ")}`,
          );
        }
      }
      await ctx.reply(lines.join("\n"), { parse_mode: "Markdown" });
    ...
    if (data.startsWith("trending_video_")) {
      ...
      const result = await tiktokAutomation.generateFromTrending({
        topic,
        contentType: "video",
      });
      if (result.success) {
        await ctx.reply(`✅ Video generated dari trending: *${topic}*`, {
          parse_mode: "Markdown",
        });
      } else {
        await ctx.reply(
          `❌ Gagal generate: ${result.error ?? "Unknown error"}`,
        );
      }
      return true;
    }
    ...
    logger.error(
      `[TikTokAutomation] Callback error: ${err instanceof Error ? err.message : String(err)}`,
    );
