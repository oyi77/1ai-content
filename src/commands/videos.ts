...
function makeDownloadUrl(jobId: string, userId: string): string {
  ...
  const token = jwt.sign({ telegramId: userId, jobId }, secret, {
    expiresIn: "30d",
  });
...
export async function videosCommand(ctx: BotContext): Promise<void> {
  try {
    ...
    if (videos.length === 0) {
      await ctx.reply(t("videos.empty", lang), {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [
            [
    ...
    if (page > 0)
      navButtons.push({ text: "◀️ Prev", callback_data: "videos_prev" });
    navButtons.push({ text: `Page ${page + 1}`, callback_data: "videos_page" });
    if (videos.length === 10)
      navButtons.push({ text: "Next ▶️", callback_data: "videos_next" });

    await ctx.reply(t("videos.list_header", lang, { count: videos.length }), {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          ...videoButtons,
          ...
          [
            {
              text: t("videos.btn_create_new", lang),
              callback_data: "create_video_new",
            },
          ],
...
export async function viewVideo(ctx: BotContext, jobId: string): Promise<void> {
  ...
  const message =
    ...
    (video.completedAt
      ? `*Completed:* ${video.completedAt.toLocaleDateString("id-ID")}\n`
      : "") +
    ...
    const textKeyboard = {
      inline_keyboard: [
        ...
        [
          {
            text: "🎬 Create Similar",
            callback_data: `create_similar_${jobId}`,
          },
        ],
        [
          {
            text: video.favorited ? "★ Unfavorite" : "☆ Favorite",
            callback_data: `video_fav_${jobId}`,
          },
              ...
              [
                {
                  ...
                  callback_data: `video_copy_${jobId}`,
                },
              ],
              [
                {
 ...
 */
export async function copyVideoUrl(
  ctx: BotContext,
  jobId: string,
): Promise<void> {
  try {
    ...
    await ctx.reply(t("videos.copy_link", lang, { url: dlUrl }), {
      parse_mode: "Markdown",
    });
  } catch (error) {
    logger.error("copyVideoUrl error:", error);
    try {
      await ctx.reply(t("error.generic", "en"));
    } catch {
      /* ignore */
    }
  }
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "✅ Yes, Delete",
                callback_data: `video_confirm_delete_${jobId}`,
              },
