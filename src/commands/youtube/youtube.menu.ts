...
import {
  runNicheCpmResearch,
  getLatestResearch,
} from "@/services/youtube/niche-research.service";
...
export async function showYouTubeMenu(ctx: BotContext): Promise<void> {
  ...
  const quarantineCount = await prisma.ytChannel.count({
...
export async function showChannelList(ctx: BotContext): Promise<void> {
  ...
  const lines = channels.map((ch, i) => {
    const statusIcon =
      ch.trafficStatus === "quarantine"
        ? "🔒"
        : ch.trafficStatus === "growing"
          ? "📈"
          : ch.trafficStatus === "established"
            ? "✅"
            : "⚪";
    ...
    reply_markup: {
      inline_keyboard: [
        ...channels.map((ch) => [
          {
            text: `📊 ${ch.channelId}`,
            callback_data: `yt_channel_${ch.channelId}`,
          },
        ]),
        [{ text: "🔙 Back", callback_data: "yt_menu_refresh" }],
      ],
...
export async function showChannelDetail(
  ...
  const statusIcon =
    channel.trafficStatus === "quarantine"
      ? "🔒"
      : channel.trafficStatus === "growing"
        ? "📈"
        : "⚪";
  const videoLines =
    recentVideos.length > 0
      ? recentVideos
          .map(
            (v) =>
              `  • ${v.status === "breakout" ? "🔥" : "📹"} ${v.title || v.videoId} [${v.status}]`,
          )
          .join("\n")
      : "  No videos yet";
...
export async function showReports(ctx: BotContext): Promise<void> {
  ...
  const totalPublished = channels.reduce(
    (sum: number, ch: { totalPublished: number }) => sum + ch.totalPublished,
    0,
  );
  const growing = channels.filter(
    (ch: { trafficStatus: string }) => ch.trafficStatus === "growing",
  ).length;
  const quarantined = channels.filter(
    (ch: { trafficStatus: string }) => ch.trafficStatus === "quarantine",
  ).length;
...
export async function showResearchResults(ctx: BotContext): Promise<void> {
  ...
  if (!research) {
    ...
    await ctx.editMessageText(
      "📭 *No research results yet.*\n\nRun research first.",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "🔍 Run Research", callback_data: "yt_menu_research" }],
            [{ text: "🔙 Back", callback_data: "yt_menu_refresh" }],
          ],
        },
      },
  ...
  const topCpm = Object.entries(research.cpmSnapshot)
    .sort(
      (a: [string, { cpmUsd: number }], b: [string, { cpmUsd: number }]) =>
        b[1].cpmUsd - a[1].cpmUsd,
    )
    .slice(0, 5)
    .map(
      ([country, data]: [string, { cpmUsd: number; trend: string }]) =>
        `• ${country}: USD ${data.cpmUsd} (${data.trend})`,
    );
  ...
  await ctx.editMessageText(text, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🔄 Re-run Research", callback_data: "yt_menu_research" }],
        [{ text: "🔙 Back", callback_data: "yt_menu_refresh" }],
      ],
    },
  });
...
export async function showQuarantine(ctx: BotContext): Promise<void> {
  ...
  const lines = quarantined.map(
    (ch: {
      ...
      const since = ch.quarantineStarted
        ? new Date(ch.quarantineStarted).toISOString().split("T")[0]
        : "N/A";
...
export async function showAgentLogs(ctx: BotContext): Promise<void> {
  const logs =
    (await (prisma as any).ytAgentTaskLog?.findMany?.({
  ...
  const lines = logs.map(
    (l: {
      ...
      const time = l.startedAt
        ? new Date(l.startedAt).toISOString().split("T")[1]?.split(".")[0]
        : "N/A";
      const icon =
