...
export async function connectCommand(ctx: BotContext): Promise<void> {
  ...
  const lines = [
    ...
    accounts.length > 0
      ? "*Connected Accounts:*\n" +
        accounts
          .map((a: { platform: string; account_name: string }) => {
  ...
  const buttons = PLATFORMS.map((p) => {
    const connected = accounts.some(
      (a: { platform: string }) => a.platform === p.id,
    );
    return [
      {
        text: connected
          ? `✅ ${p.emoji} ${p.name}`
          : `${p.emoji} Connect ${p.name}`,
        callback_data: connected
          ? `social_connected_${p.id}`
          : `social_connect_${p.id}`,
      },
    ];
  });
...
export async function publishCommand(ctx: BotContext): Promise<void> {
  ...
  const buttons = accounts.map(
    (a: { platform: string; account_name: string }) => {
      const p = PLATFORMS.find((p) => p.id === a.platform);
      return [
        {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "📅 Open Calendar",
              url: "https://content.aitradepulse.com/admin/calendar",
            },
          ],
...
export async function handleSocialCallbacks(
  ctx: BotContext,
  data: string,
): Promise<boolean> {
  const user = ctx.from;
  ...
  try {
    ...
    if (data.startsWith("social_connect_")) {
      ...
      try {
        ...
        if (resp.auth_url || resp.url) {
          ...
          await ctx.reply(
            `❌ Gagal membuat link connect untuk ${p?.name ?? platform}.`,
          );
        }
      } catch {
    ...
    if (data.startsWith("social_connected_")) {
      ...
      const p = PLATFORMS.find((p) => p.id === platform);
      await ctx.reply(
        `✅ ${p?.emoji ?? "📱"} ${p?.name ?? platform} sudah terhubung!`,
      );
      return true;
    }
    ...
    if (data.startsWith("publish_to_")) {
      ...
      const lastVideo = ctx.session?.stateData?.lastVideoPath as
        | string
        | undefined;
      if (!lastVideo) {
        await ctx.reply(
          "📤 Belum ada konten untuk di-publish.\n\nBuat video/gambar dulu, lalu kembali ke sini untuk publish.",
          {
            reply_markup: {
              inline_keyboard: [
                ...
                [
                  {
                    text: "🖼️ Buat Carousel",
                    callback_data: "carousel_regenerate",
                  },
                ],
              ],
            },
      ...
      try {
        ...
        const caption = String(
          ctx.session?.stateData?.lastCaption ??
            "Check out this content! #viral #fyp",
        );
        const { data: pubResp } = await client.post(
          "/posts",
          {
            platform,
    ...
    logger.error(
      `[Social] Callback error: ${err instanceof Error ? err.message : String(err)}`,
    );
    return true;
  }
}
