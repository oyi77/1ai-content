...
async function sendAudioFile(
  ...
  try {
    ...
    logger.error(
      `[ContentFactory] Send audio error: ${err instanceof Error ? err.message : String(err)}`,
    );
...
export async function sunoCommand(ctx: BotContext): Promise<void> {
        ...
        "Atau pilih preset di bawah:",
      {
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [
              {
                text: "🎸 Upbeat Corporate",
                callback_data: "suno_preset_corporate",
              },
            ...
            [
              {
                text: "🧘 Meditation",
                callback_data: "suno_preset_meditation",
              },
  ...
  await ctx.reply(
    "🎵 Generating music via Suno AI...\n⏳ Mohon tunggu, proses ini 1-2 menit.",
  );

  try {
    const result = await contentFactoryService.generateSunoMusic(prompt, {
      instrumental: true,
    });
    ...
    if (result.success && result.audio_path) {
      ...
      await ctx.reply("Mau posting ke sosmed?", {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "📤 Publish ke Sosmed",
                callback_data: `publish_audio_${path.basename(result.audio_path)}`,
              },
            ],
            [
              {
                text: "🔁 Buat Looping Video",
                callback_data: `loop_from_${path.basename(result.audio_path)}`,
              },
            ],
          ],
        },
      ...
      await ctx.reply(
        `❌ Gagal generate music: ${result.error || "Unknown error"}`,
      );
    }
  } catch (err: unknown) {
    logger.error(
      `[Suno] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
    await ctx.reply(
...
export async function voiceCommand(ctx: BotContext): Promise<void> {
  ...
  const isIndonesian =
    ...
    /yang|dan|di|ke|dari|untuk|dengan|ini|itu|adalah|bisa|akan|sudah|tidak|juga|ada|saya|kamu|kita/.test(
      input.toLowerCase(),
    );
  ...
  try {
    const result = await contentFactoryService.synthesizeSpeech(input, {
      language,
      voice,
    });
    ...
    if (result.success && result.audio_path) {
      ...
      await ctx.reply(
        `❌ Gagal generate voiceover: ${result.error || "Unknown error"}`,
      );
    }
  } catch (err: unknown) {
    logger.error(
...
export async function musicCommand(ctx: BotContext): Promise<void> {
  ...
  try {
    const result = await contentFactoryService.generateMusic(prompt, {
      duration: 60,
    });
...
export async function loopCommand(ctx: BotContext): Promise<void> {
  ...
  await ctx.reply("🔁 Pilih visual type untuk looping video:", {
    reply_markup: {
      inline_keyboard: [
        [
...
export async function analyzeCommand(ctx: BotContext): Promise<void> {
  ...
  try {
    const result = await contentFactoryService.analyzeChannel(channelUrl, {
      limit: 50,
    });
    ...
    if (result.success && result.channel) {
      ...
      if (strategy.what_works) {
        report += `🎯 *What Works*\n`;
        const items = Array.isArray(strategy.what_works)
          ? strategy.what_works
          : [strategy.what_works];
      ...
      if (strategy.content_ideas) {
        ...
        const ideas = Array.isArray(strategy.content_ideas)
      ...
      await ctx.reply(
        `❌ Gagal menganalisa channel: ${result.error || "Unknown error"}`,
      );
    }
  } catch (err: unknown) {
    logger.error(
      `[Analyze] Error: ${err instanceof Error ? err.message : String(err)}`,
    );
    await ctx.reply(
      "❌ Terjadi kesalahan. Pastikan Content Factory API berjalan dan yt-dlp terinstall.",
    );
  }
}
...
export async function publishCommand(ctx: BotContext): Promise<void> {
  await ctx.reply("📤 *Publish ke Sosmed*\n\n" + "Pilih platform tujuan:", {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [
