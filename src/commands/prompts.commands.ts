...
import { PROMPT_LIBRARY, TRENDING_PROMPTS, findAnyPrompt, getPromptById, getUserDailyPrompt } from "./prompts.data";
import { showNichePrompts, showPromptDetail, showCustomizePrompt, showMyPrompts, startAddCustomPrompt } from "./prompts.ui";
...
export async function dailyCommand(ctx: BotContext): Promise<void> {
  try {
    ...
    if (!userId) { await ctx.reply(t("social.unable_identify_user", "id")); return; }

    const dbUser = await UserService.findByTelegramId(BigInt(userId));
    if (!dbUser) { await ctx.reply(t("error.user_not_found", "id")); return; }
    ...
    await ctx.reply(msg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "🚀 Pakai Sekarang", callback_data: `use_prompt_${p.id}` }, { text: "💾 Simpan", callback_data: `daily_save_${p.id}` }],
...
export async function trendingCommand(ctx: BotContext): Promise<void> {
  try {
    ...
    TRENDING_PROMPTS.forEach((t, i) => {
      ...
      buttons.push([{ text: `🔥 Use #${i + 1} ${p.title}`, callback_data: `use_prompt_${p.id}` }]);
    });
    ...
    await ctx.reply(msg, {
      parse_mode: "Markdown",
      reply_markup: { inline_keyboard: [...buttons, [{ text: "📚 Browse Semua Niche", callback_data: "back_prompts" }]] },
    });
  } catch (err) {
...
export async function saveLibraryPrompt(ctx: BotContext, promptId: string): Promise<void> {
  try {
    ...
    if (!p) { await ctx.answerCbQuery(t("cb.prompt_not_found", "id")); return; }

    const telegramId = ctx.from?.id;
    if (!telegramId) { await ctx.answerCbQuery(t("error.user_not_found", "id")); return; }

    const dbUser = await UserService.findByTelegramId(BigInt(telegramId));
    if (!dbUser) { await ctx.answerCbQuery(t("error.user_not_found", "id")); return; }

    const count = await SavedPromptService.count(dbUser.id as unknown as bigint);
    if (count >= 20) { await ctx.answerCbQuery("⚠️ Max 20 prompt tersimpan. Hapus dulu yang lama."); return; }

    await SavedPromptService.save(dbUser.id as unknown as bigint, {
      title: p.title, prompt: p.prompt, niche: p.niche, source: "library", sourceId: promptId,
    });
