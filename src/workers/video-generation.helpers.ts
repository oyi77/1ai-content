...
export async function notifyProgress(telegram: Telegram, chatId: number, text: string): Promise<void> {
  try { await telegram.sendMessage(chatId, text, { disable_notification: true }); }
  catch (err) { logger.warn('Failed to send progress notification:', err); }
}

export function startTimeoutWatcher(
  telegram: Telegram, chatId: number, thresholdMs: number = 4 * 60 * 1000
): () => void {
  const timer = setTimeout(() => {
    notifyProgress(telegram, chatId, '\u23f3 Still processing... Taking longer than usual. We\u2019ll notify you when ready.');
  }, thresholdMs);
...
export function buildPrompt(description: string, platform: string, duration: number, customPrompt?: string): string {
...
export function getAspectRatio(platform: string): string {
  const ratios: Record<string, string> = {
    tiktok: '9:16', shorts: '9:16', reels: '9:16',
    facebook: '16:9', youtube: '16:9', instagram: '4:5', square: '1:1',
  };
...
export function getStyleForNiche(niche: string): string {
  const styles: Record<string, string> = {
    trading: 'professional', fitness: 'energetic', cooking: 'appetizing',
    tech: 'modern', travel: 'cinematic', education: 'clear',
  };
...
export async function concatenateVideos(inputPaths: string[], outputPath: string, niche?: string): Promise<void> {
  await VideoPostProcessing.concatenateWithTransitions(inputPaths, outputPath, {
    niche, transitionDuration: 0.5,
  });
}
