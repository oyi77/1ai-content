...
export class RetentionScheduler {
   ...
   */
  static async checkType6SubscriptionExpiring(bot: {
    telegram: Telegram;
  }): Promise<void> {
    const now = new Date();
    const triggers = [
    ...
    for (const trigger of triggers) {
      const targetDate = new Date(
        now.getTime() + trigger.daysAhead * 24 * 60 * 60 * 1000,
      );
      ...
      for (const sub of subs) {
        ...
        if (await this.canSend(sub.user.telegramId, "subscription")) {
          ...
          await this.sendMessage(
            bot,
            sub.user.telegramId,
            t(trigger.msgKey, lang, { name: sub.user.firstName }),
            lang,
          );
   ...
   */
  static async checkType1DepositNoUse(bot: {
    telegram: Telegram;
  }): Promise<void> {
    const type1Hours = await AdminConfigService.get<number[]>(
      "retention",
      "type1_trigger_hours",
      [2, 24, 72, 168],
    );
      ...
      for (const user of users) {
        if (await this.canSend(user.telegramId, "deposit")) {
          ...
          await this.sendMessage(
            bot,
            user.telegramId,
            t(msgKey, lang, { name: user.firstName }),
            lang,
          );
   ...
   * Type 2: Used free trial, no deposit
   */
  static async checkType2FreeNoDeposit(bot: {
    telegram: Telegram;
  }): Promise<void> {
    const type2Hours = await AdminConfigService.get<number[]>(
      "retention",
      "type2_trigger_hours",
      [1, 24, 72, 168],
    );
  ...
  static async checkType3UsedOnce(bot: { telegram: Telegram }): Promise<void> {
    ...
    for (const trigger of triggers) {
      const cutoff = new Date(
        Date.now() - trigger.daysAgo * 24 * 60 * 60 * 1000,
      );
      const cutoffPrev = new Date(
        Date.now() - (trigger.daysAgo + 1) * 24 * 60 * 60 * 1000,
      );
      ...
      for (const user of users) {
        if (await this.canSend(user.telegramId, "inactive")) {
          ...
          await this.sendMessage(
            bot,
            user.telegramId,
            t(trigger.msgKey, lang, { name: user.firstName }),
            lang,
          );
   ...
   */
  static async checkType4ActiveNoAffiliate(bot: {
    telegram: Telegram;
  }): Promise<void> {
    ...
    for (const count of checkPoints) {
      ...
      const usersWithVideoCount = await prisma.user.findMany({
        ...
        select: {
          ...
          _count: {
            select: {
              videos: { where: { status: { in: ["completed", "done"] } } },
            },
          },
        },
        take: 100,
      ...
      const users = usersWithVideoCount
        .filter((u) => u._count.videos === count)
        .slice(0, 30);
   ...
   */
  static async checkType7VideoExpiry(bot: {
    telegram: Telegram;
  }): Promise<void> {
    const now = new Date();
    ...
    const byUser = new Map<
      string,
      { user: (typeof expiringVideos)[0]["user"]; count: number }
    >();
    ...
    for (const { user, count } of byUser.values()) {
      ...
      if (await this.canSend(user.telegramId, "video_expiry")) {
        const lang = user.language || "id";
        const msg = t("retention.video_expiry_warning", lang, {
          count: String(count),
        });
  ...
  private static async canSend(
    ...
    return (
      categoryCount < retConfig.maxPerCategoryPer3Days &&
      dayCount < retConfig.maxPerDay
    );
  }

  private static async logSent(
    userId: bigint,
    triggerType: string,
  ): Promise<void> {
  ...
  private static async sendMessage(
    ...
    try {
      await bot.telegram.sendMessage(Number(telegramId), message, {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: t("retention.btn_generate", lang),
                callback_data: "generate_start",
              },
              {
                text: t("retention.btn_unsubscribe", lang),
                callback_data: "notif_unsubscribe",
              },
            ],
          ],
        },
      ...
      if ((err as Error).message?.includes("bot was blocked")) {
        ...
        logger.error(
          `[retention] Failed to send to ${telegramId}:`,
