...
async function processSingleScene(
  ...
  if (
    existingVideo &&
    (existingVideo.status === "failed" || existingVideo.status === "completed")
  ) {
    logger.warn(
      `Job ${jobId} already ${existingVideo.status} — skipping retry to prevent free generation`,
    );
    return;
  }
  ...
  if (!result.success || !result.videoUrl) {
    ...
    if (existingVideo?.status === "failed") {
      logger.warn(
        `Job ${jobId} already failed/refunded — skipping duplicate refund`,
      );
      return;
    }
    const creditCost =
      job.data.creditCost ?? (await getVideoCreditCostAsync(duration));
    ...
    if (!lockAcquired) {
      logger.warn(
        `Refund lock already held for job ${jobId} — skipping duplicate refund`,
      );
    } else {
    ...
    const userMessage = actionableError(result.error || "Generation failed", {
      jobId,
    });
  ...
  await notifyProgress(
    telegram,
    chatId,
    "\u2705 Video generated! Downloading and processing...",
  );
  ...
  try {
    ...
    const qcResult = await QualityCheckService.scoreVideo(
      localPath,
      niche,
      duration,
      !!referenceImage,
    ...
    if (!qcResult.passable && !isRetry) {
      logger.warn(
        `[QualityCheck] Video ${jobId} scored ${qcResult.score}/10 -- retrying with different provider`,
      );
      ...
      await notifyProgress(
        telegram,
        chatId,
        "\ud83d\udd04 Improving quality... Regenerating video.",
      );
      ...
      if (retryResult.success && retryResult.videoUrl) {
        ...
        logger.info(
          `[QualityCheck] Retry succeeded via ${retryResult.provider} for job ${jobId}`,
        );
      } else {
        logger.warn(
          `[QualityCheck] Retry failed for ${jobId}, delivering original`,
        );
        await downloadVideo(result.videoUrl, localPath);
      }
    } else {
      logger.info(
        `[QualityCheck] Video ${jobId} passed with score ${qcResult.score}/10`,
      );
    }
  } catch (qcErr) {
    logger.warn(
      `[QualityCheck] Quality check error for ${jobId}, delivering as-is:`,
      (qcErr as Error).message,
    );
  }
  ...
  if (enableVO || enableSubtitles) {
    ...
    await notifyProgress(
      telegram,
      chatId,
      "\ud83d\udce6 Almost ready! Preparing delivery...",
    );
  }

  await VideoService.setOutput(jobId, {
    videoUrl: result.videoUrl,
    downloadUrl: deliveryPath,
  });
  ...
  if (job.data.cacheAsTemplate && job.data.cacheNiche) {
    try {
      ...
      logger.warn("Failed to cache template video", {
  ...
  if (job.data.campaignGroupId) {
    await handleCampaignJobComplete(
      telegram,
      chatId,
      job.data.campaignGroupId,
      job.data.campaignTotal || 5,
      deliveryPath,
      result.videoUrl || "",
      niche,
      job.data.userId,
  ...
  GamificationService.recordGenerate(telegramId, {
    ...
    logger.warn("Gamification recordGenerate failed (single-scene)", err),
  );
}
...
async function processExtendedScenes(
  ...
  if (
    existingVideoExt &&
    (existingVideoExt.status === "failed" ||
      existingVideoExt.status === "completed")
  ) {
    logger.warn(
      `Extended job ${jobId} already ${existingVideoExt.status} — skipping retry`,
    );
    return;
  }
  ...
  async function generateSceneWithRetry(
    sceneIndex: number,
    useExtend: boolean,
    lastUuidRef: string | null,
  ): Promise<{
    result:
      | import("@/services/video-fallback.service").VideoFallbackResult
      | import("@/services/geminigen.service").VideoGenerationResult;
    scenePath: string;
  }> {
    const scene = storyboard[sceneIndex];
    const scenePath = path.join(
      VIDEO_DIR,
      `${jobId}_scene_${sceneIndex + 1}.mp4`,
    );
    const prompt = buildPrompt(
      scene.description,
      platform,
    ...
    logger.info(
      `Generating scene ${sceneIndex + 1}/${scenes} for job ${jobId}: ${scene.description}`,
    );
    let result:
      | import("@/services/video-fallback.service").VideoFallbackResult
      | import("@/services/geminigen.service").VideoGenerationResult;
    const sceneRef =
      job.data.userImages?.find((u) => u.sceneIndex === sceneIndex)?.url ||
      referenceImage;
    ...
    if (sceneIndex === 0) {
      ...
      try {
        result = await GeminiGenService.generateExtend({
          prompt,
          refHistory: lastUuidRef,
        });
      } catch (extendErr) {
        logger.warn(
          `Scene ${sceneIndex + 1} extend failed, falling back to standalone: ${(extendErr as Error).message}`,
        );
        result = await generateVideoWithFallback({
          prompt,
          duration: scene.duration,
    ...
    if (!result.success || !result.videoUrl) {
      logger.warn(
        `Scene ${sceneIndex + 1}/${scenes} failed on first attempt: ${result.error}. Retrying...`,
      );
      result = await generateVideoWithFallback({
        prompt,
        duration: scene.duration,
    ...
    if (!result.success || !result.videoUrl) {
      throw new ProviderError(
        "scene-generation",
        `Scene ${sceneIndex + 1} failed after 2 attempts: ${result.error}`,
      );
    }
    try {
  ...
  try {
    ...
    await notifyProgress(
      telegram,
      chatId,
      `\ud83c\udfac Scene 1/${scenes} complete (${Math.round((1 / scenes) * 100)}%)`,
    );
  } catch (err) {
    cancelTimeout();
    const existingVideo = await VideoService.getByJobId(jobId);
    if (existingVideo?.status === "failed") {
      logger.warn(
        `Job ${jobId} already failed/refunded (scene 1) — skipping duplicate refund`,
      );
      return;
    }
    const creditCost =
  ...
  for (let batchStart = 1; batchStart < scenes; batchStart += BATCH_SIZE) {
    ...
    for (let i = batchStart; i < batchEnd; i++) {
      batchPromises.push(
        generateSceneWithRetry(i, false, null).then(({ scenePath }) => ({
          sceneIndex: i,
          scenePath,
        })),
      );
    ...
    for (const settled of batchResults) {
      if (settled.status === "rejected") {
        batchFailures++;
        logger.warn(
          `Batch scene failed (will skip): ${settled.reason?.message}`,
        );
        continue;
      }
    ...
    if (batchFailures > 0) {
      await notifyProgress(
        telegram,
        chatId,
        `⚠️ ${batchFailures} scene(s) skipped — continuing with remaining scenes.`,
      );
    }
    ...
    await VideoService.updateProgress(
      jobId,
      Math.round((completedCount / scenes) * 80),
    );
    for (let i = batchStart; i < batchEnd; i++) {
      await notifyProgress(
        telegram,
        chatId,
        `\ud83c\udfac Scene ${i + 1}/${scenes} complete (${Math.round(((i + 1) / scenes) * 100)}%)`,
      );
    }
  }
  ...
  if (successfulScenes.length === 0) {
    ...
    if (existingVideo?.status === "failed") {
      logger.warn(
        `Job ${jobId} already failed/refunded (all scenes) — skipping duplicate refund`,
      );
      return;
    }
    const creditCost =
    ...
    await telegram.sendMessage(
      chatId,
      t("gen.video_failed_refund", job.data.language || "id"),
    );
    return;
  }
  ...
  if (failedSceneCount > 0) {
    const creditCost =
      job.data.creditCost ?? (await getVideoCreditCostAsync(duration));
    const refundAmount =
      Math.round(((creditCost * failedSceneCount) / scenes) * 100) / 100;
    if (refundAmount > 0) {
      const partialRefundLockKey = `refund-lock:${jobId}:partial`;
      const partialLockAcquired = await redis.set(
        partialRefundLockKey,
        "1",
        "EX",
        3600,
        "NX",
      ...
      if (!partialLockAcquired) {
        logger.warn(
          `Partial refund lock already held for job ${jobId} — skipping duplicate partial refund`,
        );
      } else {
        await UserService.refundCredits(
          telegramId,
          refundAmount,
          jobId,
          `${failedSceneCount}/${scenes} scenes failed`,
        ).catch((err) =>
          logger.error("CRITICAL: partial scene refund failed", {
            jobId,
            refundAmount,
            err,
          }),
      ...
      await telegram
        .sendMessage(
          chatId,
          t("worker.partial_refund", job.data.language || "id", {
            amount: refundAmount,
  ...
  logger.info(
    `Concatenating ${successfulScenes.length}/${scenes} scenes for job ${jobId} (niche: ${niche})...`,
  );
  ...
  try {
    ...
    logger.warn(
      `🧹 Multi-scene watermark removal skipped: ${(wmErr as Error).message}`,
    );
  }
  ...
  try {
    ...
    logger.warn(
      `Post-processing failed for job ${jobId}, using raw concat: ${(ppErr as Error).message}`,
    );
  ...
  try {
    ...
    if (!qcResult.passable) {
      logger.warn(
        `[QualityCheck] Multi-scene video ${jobId} scored ${qcResult.score}/10 -- delivering as-is (no retry for multi-scene)`,
      );
    } else {
      logger.info(
        `[QualityCheck] Multi-scene video ${jobId} passed with score ${qcResult.score}/10`,
      );
    }
  } catch (qcErr) {
    logger.warn(
      `[QualityCheck] Quality check error for multi-scene ${jobId}:`,
      (qcErr as Error).message,
    );
  }
  ...
  GamificationService.recordGenerate(telegramId, {
    ...
    logger.warn("Gamification recordGenerate failed (multi-scene)", err),
  );
