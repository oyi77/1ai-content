...
import { CATEGORY_TO_VOICE, AI_VOICE_PROFILES, MARKETING_HOOKS, MARKETING_CTAS } from '@/config/audio-subtitle-engine';
...
export async function generateVOScriptWithAI(
  ...
  const sceneDescriptions = storyboard.map((s, i) => `Scene ${i + 1} (${s.duration}s): ${s.description}`).join('\n');
  ...
  if (promptsCfg?.voNarration) {
    prompt = promptsCfg.voNarration
      .replace(/{niche}/g, niche).replace(/{duration}/g, String(totalDuration))
      .replace(/{language}/g, langLabel).replace(/{sceneDescriptions}/g, sceneDescriptions)
...
export function generateVOScriptTemplate(
  ...
  if (language === 'en') {
    ...
    const middle = sceneNarrations.length > 0 ? sceneNarrations.slice(0, 3).join('. ') : 'Discover something special today.';
    return `Check this out. ${middle}. Don't miss it — see the details now.`;
  }

  const sceneNarrations = storyboard.map(s => s.description).filter(Boolean);
  const middle = sceneNarrations.length > 0 ? sceneNarrations.slice(0, 3).join('. ') : 'Temukan sesuatu yang istimewa hari ini.';
