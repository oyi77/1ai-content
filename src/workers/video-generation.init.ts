...
import {
  processSingleScene,
  processExtendedScenes,
} from "./video-generation.scene";

let workerInstance: Worker<VideoGenerationJobData> | null = null;

export function startVideoWorker(bot: {
  telegram: Telegram;
}): Worker<VideoGenerationJobData> {
  if (workerInstance) {
  ...
  workerInstance = new Worker<VideoGenerationJobData>(
    ...
    (job: Job<VideoGenerationJobData>) =>
      runWithCorrelation(async () => {
        logger.info(
          `Processing video job ${job.id} (jobId=${job.data.jobId}, scenes=${job.data.scenes})`,
        );
        ...
        try {
          ...
          if (intercepted) {
            logger.info(
              `Job ${job.data.jobId}: user ${telegramId} is intercepted — pausing for admin media`,
            );

            await InterceptService.logEvent(
              telegramId,
              "generation_started",
              `Job started: ${job.data.jobId}`,
              {
            ...
            const result = await InterceptService.waitForMedia(
              job.data.jobId,
              1800,
            );
            ...
            if (!result) {
              const creditCost =
                job.data.creditCost ??
                (await getVideoCreditCostAsync(job.data.duration));
            ...
            await VideoService.upsertForInterception(
              job.data.jobId,
              telegramId,
              mediaUrl,
            );
            ...
            if (mediaType === "image") {
              await telegram.sendPhoto(job.data.chatId, mediaUrl, {
                caption,
                parse_mode: "Markdown",
                reply_markup: replyMarkup,
              });
            } else {
              await telegram.sendVideo(job.data.chatId, mediaUrl, {
                caption,
                parse_mode: "Markdown",
                reply_markup: replyMarkup,
              });
          ...
          logger.error(
            `Video job ${job.id} failed with unhandled error:`,
            error,
          );
          ...
          try {
            ...
            if (existingVideo?.status === "failed") {
              logger.warn(
                `Job ${job.data.jobId} already failed/refunded (catch) — skipping duplicate refund`,
              );
            } else {
              const telegramId = BigInt(job.data.userId);
              const creditCost =
              ...
              if (!lockAcquired) {
                logger.warn(
                  `Refund lock already held for job ${job.data.jobId} — skipping duplicate refund`,
                );
              } else {
                await UserService.refundCredits(
                  telegramId,
              ...
              const workerUserMessage = actionableError(
                (error as Error).message,
                { jobId: job.data.jobId },
              );
              await telegram.sendMessage(
  ...
  workerInstance.on("error", (err) => {
    ...
    sendAdminAlert("warning", "Video Worker Error", {
      error: (err as Error)?.message?.slice(0, 300),
    });
  });
