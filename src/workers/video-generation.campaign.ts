...
import { VIDEO_DIR, downloadVideo, concatenateVideos } from './video-generation.helpers';
  ...
  try {
    ...
    for (let i = 0; i < rawEntries.length; i++) {
      ...
      if (entry.videoUrl) {
        ...
        try { await downloadVideo(entry.videoUrl, dlPath); tmpPaths.push(dlPath); }
        catch (dlErr) { logger.warn(`Campaign ${campaignGroupId}: failed to download part ${i}:`, dlErr); }
      }
    }
    ...
    if (tmpPaths.length === 1) {
      ...
      try {
        ...
        await telegram.sendVideo(chatId, { source: mergedPath }, {
          ...
          parse_mode: 'Markdown',
          reply_markup: { inline_keyboard: [[{ text: '🎬 Buat Campaign Lagi', callback_data: 'action_campaign' }], [{ text: '🏠 Menu Utama', callback_data: 'main_menu' }]] },
        });
      } catch (mergeErr) {
