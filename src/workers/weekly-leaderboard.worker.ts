...
export class WeeklyLeaderboardService {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(3, leaderboard.length); i++) {
        ...
        if (entry.creditReward > 0) {
          ...
          logger.info(
            `[leaderboard] Awarded ${entry.creditReward} credit to user ${entry.userId} (rank ${i + 1})`,
          );
      ...
      logger.info(
        `[leaderboard] Weekly cycle complete. ${leaderboard.length} users ranked.`,
      );
    } catch (err) {
  ...
  static async scheduleNext(): Promise<void> {
    ...
    logger.info(
      `[leaderboard] Next run scheduled: ${nextMonday.toISOString()}`,
    );
  }
}
