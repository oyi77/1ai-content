  ...
  try {
    ...
    const replyMarkup = {
      inline_keyboard: [
        ...
        [
          {
            text: t("video.btn_publish", lang),
            callback_data: `publish_video_${jobId}`,
          },
        ],
        [
          {
            text: t("video.btn_good", lang),
            callback_data: `feedback_good_${jobId}`,
          },
          {
            text: t("video.btn_needs_work", lang),
            callback_data: `feedback_bad_${jobId}`,
          },
        ],
        [
          {
            text: t("video.btn_create_another", lang),
            callback_data: "create_video_new",
          },
          {
            text: t("video.btn_my_videos", lang),
    ...
    if (fs.existsSync(localPath)) {
      await telegram.sendVideo(
        chatId,
        { source: localPath },
        { caption, parse_mode: "Markdown", reply_markup: replyMarkup },
      );
    ...
    if (dbUser) {
      const videoCount = await prisma.video.count({
        where: { userId: dbUser.telegramId },
      });
      if (videoCount <= 1) {
        const tips = t("video.first_video_tips", lang);
        await telegram
          .sendMessage(chatId, tips, { parse_mode: "Markdown" })
          .catch(() => {});
      }
    }
    ...
    logger.error(
      `Failed to send video notification for job ${jobId}:`,
      sendErr,
    );
  }
}
