  ...
  static async analyze(videoUrl: string): Promise<VideoAnalysisResult> {
    ...
    try {
      ...
      const isSocialPlatform =
        /tiktok\.com|instagram\.com|youtube\.com|youtu\.be|twitter\.com|x\.com|facebook\.com|fb\.watch|pinterest\.com|bilibili\.com/i.test(
          videoUrl,
        );
      try {
        logger.info(
          `[VideoAnalysis] Downloading video (${isSocialPlatform ? "yt-dlp" : "wget"}): ${videoUrl.slice(0, 80)}`,
        );
        if (isSocialPlatform) {
          const ytdlpBin = "/home/openclaw/.local/bin/yt-dlp";
          const ytdlpCmd = require("fs").existsSync(ytdlpBin)
            ? ytdlpBin
            : "yt-dlp";
          await execFile(
            ytdlpCmd,
        ...
        logger.warn(
          `[VideoAnalysis] Download failed: ${(err as Error).message}`,
        );
      ...
      try {
        ...
        logger.warn(
          `[VideoAnalysis] ffprobe failed: ${(err as Error).message}`,
        );
      }
      ...
      try {
        ...
        logger.warn(
          `[VideoAnalysis] ffmpeg frame extraction failed: ${(err as Error).message}`,
        );
      }
      ...
      if (!getConfig().GEMINI_API_KEY) {
        logger.warn(
          "[VideoAnalysis] GEMINI_API_KEY not set — using config-driven fallback chain",
        );
        analysisResult = await VideoAnalysisService._analyzeViaFallbackChain(
          keyFramePaths,
          videoUrl,
        );
        if (!analysisResult.totalDuration)
          analysisResult.totalDuration = totalDuration;
        analysisResult.keyFramePaths = keyFramePaths;
      } else {
        analysisResult = await VideoAnalysisService._callGemini(
          tempPath,
          videoUrl,
          keyFramePaths,
        );
...
Break the video into 1 scene per ~5 seconds (max 8 scenes total). Make each prompt detailed enough to regenerate the scene independently.`;
    ...
    const requestBody = {
      contents: [{ parts: [inlinePart, { text: systemPrompt }] }],
    ...
    try {
      ...
      logger.warn(
        `[VideoAnalysis] Gemini API failed (${(err as Error).message}), using config-driven fallback chain`,
      );
      return VideoAnalysisService._analyzeViaFallbackChain(
        keyFramePaths,
        originalUrl,
      );
    }

    return VideoAnalysisService._parseResponse(
      responseText,
      keyFramePaths,
      originalUrl,
    );
  ...
  private static _parseResponse(
    ...
    try {
      ...
      logger.warn(
        `[VideoAnalysis] JSON parse failed: ${(parseErr as Error).message}`,
      );
  ...
  private static async _analyzeViaUrl(
    videoUrl: string,
  ): Promise<VideoAnalysisResult> {
    const requestBody = {
      contents: [
    ...
    try {
      ...
      return {
        ...
        storyboard: storyboard.length
          ? storyboard
          : buildFallbackResult(videoUrl).storyboard,
        keyFramePaths: [],
      };
    } catch {
  ...
  private static async _analyzeViaFallbackChain(
    ...
    const tasksConfig = await AIConfigService.getTasksConfig().catch(
      () => null,
    );
    const fallback1 = tasksConfig?.transcriptFallback1 ?? {
      provider: "omniroute",
      model: "antigravity/gemini-2.5-flash",
    };
    const fallback2 = tasksConfig?.transcriptFallback2 ?? {
      provider: "groq",
    ...
    for (const cfg of chain) {
      try {
        const result = await VideoAnalysisService._analyzeViaProvider(
          cfg.provider,
          cfg.model,
          keyFramePaths,
          videoUrl,
        ...
        logger.warn(
          `[VideoAnalysis] Fallback provider ${cfg.provider}/${cfg.model} failed: ${(err as Error).message}`,
        );
      }
    }

    logger.warn(
      "[VideoAnalysis] All fallback providers failed, returning hardcoded fallback",
    );
  ...
  private static async _analyzeViaProvider(
    ...
    if (provider === "groq") {
      return VideoAnalysisService._analyzeViaGroq(
        keyFramePaths,
        videoUrl,
        model,
      );
    }
    return VideoAnalysisService._analyzeViaOmniRoute(
      keyFramePaths,
      videoUrl,
      model,
    );
...
{"niche":"one of: fitness,food,travel,business,fashion,education,tech,beauty,general","style":"brief visual style","totalDuration":15,"transcript":"spoken words if any","storyboard":[{"scene":1,"startTime":0,"duration":15,"description":"detailed visual description","prompt":"cinematic AI video generation prompt for recreating this scene"}]}`;
    ...
    if (keyFramePaths.length > 0) {
      for (const framePath of keyFramePaths.slice(0, 3)) {
        try {
          ...
          if (storyboard.length) {
            logger.info(
              `[VideoAnalysis] OmniRoute fallback succeeded with frame ${framePath}`,
            );
            return {
              success: true,
              niche: parsed.niche || "general",
          ...
          logger.warn(
            `[VideoAnalysis] OmniRoute frame analysis failed: ${(frameErr as Error).message}`,
          );
        }
      }
    }
    ...
    try {
      ...
      if (result.success && result.content) {
        const jsonStr = extractJSON(result.content);
        const parsed = JSON.parse(jsonStr);
        const storyboard: AnalyzedScene[] = (parsed.storyboard || []).map(
      ...
      logger.warn(
        `[VideoAnalysis] OmniRoute text fallback failed: ${(textErr as Error).message}`,
      );
    }

    logger.warn(
      "[VideoAnalysis] OmniRoute analysis failed, trying Groq vision fallback",
    );
  ...
  private static async _analyzeViaGroq(
    ...
    if (!apiKey || keyFramePaths.length === 0)
      return buildFallbackResult(videoUrl);
...
{"niche":"one of: fitness,food,travel,business,fashion,education,tech,beauty,general","style":"brief visual style","totalDuration":15,"transcript":"spoken words if any","storyboard":[{"scene":1,"startTime":0,"duration":15,"description":"detailed visual description","prompt":"cinematic AI video generation prompt for recreating this scene"}]}`;
    ...
    for (const framePath of keyFramePaths.slice(0, 2)) {
      try {
        ...
        if (storyboard.length) {
          logger.info(
            `[VideoAnalysis] Groq vision fallback succeeded with frame ${framePath}`,
          );
          return {
            success: true,
            niche: parsed.niche || "general",
        ...
        logger.warn(
          `[VideoAnalysis] Groq vision frame failed: ${(err as Error).message}`,
        );
      }
    }

    logger.warn(
      "[VideoAnalysis] All analysis methods failed, returning fallback result",
    );
    return buildFallbackResult(videoUrl);
  }
}
