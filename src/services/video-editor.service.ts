...
export class VideoEditorService {
  ...
  async changeSpeed(
    ...
    await execFileAsync(
      "ffmpeg",
      ["-y", "-i", inputPath, "-vf", videoFilter, "-af", audioFilter, output],
      { timeout: 120000 },
    );
  ...
  async rotate(
    ...
    await execFileAsync(
      ...
      [
        ...
        inputPath,
        "-vf",
        transposeMap[degrees] || "transpose=1",
        output,
      ],
      { timeout: 120000 },
    );
  ...
  async flip(
    ...
    await execFileAsync(
      "ffmpeg",
      ["-y", "-i", inputPath, "-vf", filter, output],
      { timeout: 120000 },
    );
    ...
    if (audio) {
      ...
      await execFileAsync(
        ...
        [
          ...
          audio,
          "-filter_complex",
          `[1:a]volume=${audioVolume}[a];[0:a][a]amix=inputs=2:duration=first[out]`,
          "-map",
          "0:v",
          "-map",
          "[out]",
  ...
  async addWatermark(options: WatermarkOptions): Promise<string> {
    ...
    await execFileAsync(
      ...
      [
        ...
        watermarkPath,
        "-filter_complex",
        `[1:v]scale=iw*${scale}:ih*${scale},format=rgba,colorchannelmixer=aa=${opacity}[wm];[0:v][wm]overlay=${positionMap[position]}`,
        "-c:a",
        "copy",
        output,
      ],
  ...
  async addTextOverlay(
    ...
    await execFileAsync(
      ...
      [
        ...
        inputPath,
        "-vf",
        `drawtext=text='${text.replace(/'/g, "\\'")}':fontsize=${fontSize}:fontcolor=${fontColor}:x=(w-tw)/2:y=${yPosition}:box=1:boxcolor=${backgroundColor}:boxborderw=5`,
        "-c:a",
        "copy",
        output,
      ],
  ...
  async getVideoInfo(inputPath: string): Promise<{
    ...
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        "v:0",
        "-show_entries",
        "stream=width,height,r_frame_rate,bit_rate,codec_name",
        "-show_entries",
