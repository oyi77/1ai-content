...
async function ensureLocalImage(
  refImage: string | null | undefined,
): Promise<string | null> {
  if (!refImage) return null;
  // Already a local file that exists
  if (
    !refImage.startsWith("http") &&
    fs.existsSync(refImage) &&
    fs.statSync(refImage).size > 0
  ) {
    return refImage;
  }
  ...
  if (refImage.startsWith("http")) {
    try {
      ...
      const tmpPath = path.join(
        os.tmpdir(),
        `ref_img_${Date.now()}_${secureRandomString(6).toLowerCase()}.jpg`,
      );
      const response = await axios.get(refImage, {
...
export async function generateVideoWithFallback(
  ...
  import("./video-fallback/providers/providers-registry").VideoFallbackResult
> {
  ...
  const resolvedParams =
    resolvedRef !== params.referenceImage
  ...
  try {
    const { getProviders } =
      await import("./video-fallback/providers/providers-registry.js");
    ...
    const promptMaxChars = await AdminConfigService.getAiParam(
      "prompt_max_chars",
      800,
    );
    ...
    for (const provider of providersToTry) {
      ...
      if (params.duration <= provider.maxDuration) {
        ...
        try {
          const scenesNeeded = Math.ceil(
            params.duration / provider.maxDuration,
          );
          ...
          logger.warn(
            `${provider.name} multi-scene error: ${(error as Error).message}`,
          );
        }
      }
    }
