...
export async function validateAudio(
  ...
  else if (info.sample_rate < getMinSampleRate())
    failures.push(`Sample rate ${info.sample_rate} < ${getMinSampleRate()}`);

  if (info.duration && expectedDurationSec > 0) {
    const tolerance = getConfig().YT_AUDIO_DURATION_TOLERANCE_PCT || 0.1;
    const diff =
      Math.abs(info.duration - expectedDurationSec) / expectedDurationSec;
    if (diff > tolerance)
      failures.push(
        `Audio duration ${info.duration}s differs ${(diff * 100).toFixed(0)}% from expected ${expectedDurationSec}s`,
      );
  }
...
export async function validateVideo(
  ...
  if (info.width && info.width < getMinVideoWidth())
    failures.push(`Video width ${info.width} < ${getMinVideoWidth()}`);
  if (info.height && info.height < getMinVideoHeight())
    failures.push(`Video height ${info.height} < ${getMinVideoHeight()}`);
  ...
  try {
    ...
    if (stat.size > maxBytes)
      warnings.push(
        `Video file ${(stat.size / 1024 / 1024).toFixed(0)}MB > ${getMaxVideoFileSizeMb()}MB limit`,
      );
  } catch {
    /* ignore */
  }
  ...
  if (info.duration && expectedDurationSec > 0) {
    ...
    if (Math.abs(info.duration - expectedDurationSec) > toleranceSec) {
      failures.push(
        `Video duration ${info.duration}s differs > ${toleranceSec}s from expected`,
      );
    }
  }
...
export async function validateThumbnail(
  ...
  if (info.width && info.width < getMinThumbWidth())
    failures.push(`Thumbnail width ${info.width} < ${getMinThumbWidth()}`);
  if (info.height && info.height < getMinThumbHeight())
    failures.push(`Thumbnail height ${info.height} < ${getMinThumbHeight()}`);
  ...
  try {
    ...
    if (stat.size > maxBytes)
      failures.push(
        `Thumbnail ${(stat.size / 1024 / 1024).toFixed(1)}MB > 2MB`,
      );
  } catch {
    /* ignore */
  }
...
export function validateSeo(
  ...
  if (title.length > getMaxTitleLength())
    failures.push(`Title ${title.length} chars > ${getMaxTitleLength()}`);
  if (tags.length < getMinTags())
    warnings.push(`Tags ${tags.length} < recommended ${getMinTags()}`);
  if (tags.length > getMaxTags())
    warnings.push(`Tags ${tags.length} > max ${getMaxTags()}`);
  if (description.length < (getConfig().YT_MIN_DESCRIPTION_LENGTH || 200))
