...
export async function checkQuarantineEligibility(
  ...
  const trafficDropPct =
    previousViews > 0 ? (previousViews - recentViews) / previousViews : 0;
  ...
  if (isAgeTrigger) {
    return {
      ...
      channelAgeDays: ageDays,
      trafficDropPct,
      note: "Traffic not yet dropped significantly",
    };
  }
  if (
    isTrafficDrop &&
    ageDays > (getConfig().YT_QUARANTINE_EARLY_MIN_AGE || 150)
  ) {
    return {
      eligible: true,
      ...
      channelAgeDays: ageDays,
      trafficDropPct,
      note: "Early quarantine due to traffic drop",
    };
  }
...
export async function checkRecovery(channelId: string): Promise<boolean> {
  ...
  const recentViews = await getAvgViews(channelId, 14);
  const preQuarantineViews = await getAvgViewsBeforeDate(
    channelId,
    channel.quarantineStarted,
  );
...
export async function handleFailedRecovery(channelId: string): Promise<void> {
  ...
  logger.warn(
    `[quarantine] ⚠️ ${channelId} failed recovery — needs manual intervention`,
  );
}

async function getAvgViews(
  channelId: string,
  days: number,
  offsetDays = 0,
): Promise<number> {
  const since = new Date(
    Date.now() - (days + offsetDays) * 24 * 60 * 60 * 1000,
  );
  ...
  return (
    metrics.reduce(
      (sum: number, m: { views: number | null }) => sum + (m.views || 0),
      0,
    ) / metrics.length
  );
}

async function getAvgViewsBeforeDate(
  channelId: string,
  before: Date,
): Promise<number> {
  const videos = await prisma.ytPublishedVideo.findMany({
