...
export class TripayService {
  ...
  static async createTransaction(
    params: TripayCreatePaymentParams,
  ): Promise<TripayPayment> {
    try {
      ...
      const response = await axios.post(
        `${this.getBaseUrl()}/transaction/create`,
        payload,
        {
      ...
      logger.error(
        "Tripay create transaction error:",
  ...
  static async handleCallback(
    ...
    try {
      const { merchant_ref = "", status } = callbackData as {
        merchant_ref?: string;
        status?: string;
      };
      ...
      if (newStatus === "success") {
        ...
        if (transaction.type === "subscription") {
          ...
          logger.info(
            `Subscription activated: ${plan}/${billingCycle} for user ${transaction.userId} via Tripay`,
          );
        } else {
          ...
          logger.info(
            `Added ${credits} credits to user ${transaction.userId} via Tripay (tier: ${plan?.tier || "unchanged"})`,
          );
        }
        ...
        await ReferralService.processCommissions(
          ...
          logger.warn("Tripay referral commission failed (non-blocking):", err),
        );
        ...
        try {
          ...
          logger.warn(
            `Analytics tracking failed for Tripay (non-blocking): ${analyticsError}`,
          );
        }
        ...
        if (refundResult.count === 1) {
          ...
          if (credits > 0) {
            ...
            logger.info(
              `Tripay refund: reversed ${decrementAmount} credits for user ${transaction.userId} (order ${merchant_ref})`,
            );
          }
        } else {
          logger.warn(
            `Tripay refund for ${merchant_ref} — transaction was not in success state, skipping credit reversal`,
          );
        }
  ...
  static async checkTransaction(
    reference: string,
  ): Promise<TripayTransactionResponse | null> {
    try {
      const response = await axios.get(
        `${this.getBaseUrl()}/transaction/detail`,
        {
          headers: this.getHeaders(),
          params: { reference },
        },
