...
export class UserCreditsService {
  ...
  static async grantCredits(
    ...
    logger.info(
      `Granting ${amount} credits to user ${userId}. Reason: ${reason}`,
    );
    // Add the credits
  ...
  static async grantWelcomeBonus(telegramId: bigint): Promise<boolean> {
    ...
    try {
      ...
      if (tx) {
        logger.info(
          `Welcome bonus granted to user ${telegramId} (+${bonus} credits)`,
        );
      }
      return tx;
    } catch (err) {
  ...
  static async deductCredits(
    ...
    if (remaining < 5) {
      UserCreditsService.sendLowCreditWarning(
        telegramId,
        remaining,
        updated.language || "id",
      ).catch((err) =>
        logger.warn("Failed to send low credit warning", {
          error: err.message,
        }),
      );
    }
  ...
  private static async sendLowCreditWarning(
    ...
    const sent = await UserTelegramService.sendMessage(telegramId, message, {
      parse_mode: "Markdown",
    });
    ...
    logger.info(
      `Low credit warning sent to user ${telegramId} (${remaining} remaining)`,
    );
  }
  ...
  static async queueRefundRetry(
    ...
    await redis
      .lpush("refund_retry", entry)
      .catch((err) =>
        logger.error("Failed to queue refund retry", { error: err.message }),
      );
    logger.warn(
      `Refund queued for retry: ${jobId} (${amount} credits for user ${telegramId})`,
    );
  }
  ...
  static async processRefundRetries(): Promise<number> {
    ...
    for (let i = 0; i < maxBatch; i++) {
      ...
      try {
        ...
        logger.info(
          `Refund retry succeeded: ${entry.jobId} (${entry.amount} credits for user ${entry.telegramId})`,
        );
        processed++;
      } catch (err) {
        ...
        try {
          ...
          if (entry.attempts < 5) {
            ...
            logger.warn(
              `Refund retry failed (attempt ${entry.attempts}/5), re-queued: ${entry.jobId}`,
            );
          } else {
            logger.error(
              `CRITICAL: Refund permanently failed after 5 attempts: ${raw}`,
              err,
            );
            sendAdminAlert("critical", "Refund Permanently Failed", {
              entry: raw,
              error: String(err),
            });
          }
        } catch (err) {
          logger.debug("Parse failed, entry lost:", err);
        }
      }
    }
    return processed;
  ...
  static async expireStaleCredits(telegram?: Telegram): Promise<number> {
    ...
    for (const user of expired) {
      const purchased = Math.max(
        0,
        Number(user.creditBalance) - (user.subscriptionCredits ?? 0),
      );
