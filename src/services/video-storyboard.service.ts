...
export const NICHE_TEMPLATES = {
  fnb: {
    ...
    promptStyle:
      "appetizing, warm lighting, steam, fresh ingredients, close-up shots, sizzling, pouring, plating",
    storyboardTemplate: [
      {
        scene: 1,
        duration: 3,
        type: "hook",
        description: "Close-up of signature dish/drink",
      },
      {
        scene: 2,
        duration: 5,
        type: "process",
        description: "Cooking/preparation action",
      },
      {
        scene: 3,
        duration: 4,
        type: "result",
        description: "Final presentation with steam/garnish",
      },
      {
        scene: 4,
        duration: 3,
        type: "cta",
        description: "Restaurant logo + promo",
      },
    ],
  },
  realestate: {
    name: "🏠 Real Estate",
    promptStyle:
      "wide angle, bright natural lighting, spacious feel, luxury amenities, drone shots, walkthrough",
    storyboardTemplate: [
      {
        scene: 1,
        duration: 3,
        type: "hook",
        description: "Exterior curb appeal",
      },
      {
        scene: 2,
        duration: 5,
        type: "tour",
        description: "Living room walkthrough",
      },
      {
        scene: 3,
        duration: 4,
        type: "features",
        description: "Kitchen & amenities highlight",
      },
      {
        scene: 4,
        duration: 4,
        type: "bedroom",
        description: "Master bedroom/bathroom",
      },
      {
        scene: 5,
        duration: 3,
        type: "cta",
        description: "Agent info + contact",
      },
    ],
  },
  product: {
    ...
    promptStyle:
      "studio lighting, product showcase, unboxing style, lifestyle context, detail shots, 360 rotation",
    storyboardTemplate: [
      ...
      },
      {
        ...
        duration: 4,
        type: "benefits",
        description: "Usage in daily life",
      },
  ...
  car: {
    ...
    promptStyle:
      "cinematic automotive, showroom lighting, exterior angles, interior luxury, engine detail, driving shots",
    storyboardTemplate: [
      {
        scene: 1,
        duration: 3,
        type: "hook",
        description: "Exterior front 3/4 angle",
      },
      {
        scene: 2,
        duration: 4,
        type: "exterior",
        description: "Walkaround exterior",
      },
      {
        scene: 3,
        duration: 4,
        type: "interior",
        description: "Interior luxury features",
      },
      {
        scene: 4,
        duration: 3,
        type: "engine",
        description: "Engine/performance specs",
      },
      {
        scene: 5,
        duration: 3,
        type: "cta",
        description: "Dealership info + price",
      },
    ],
  },
  beauty: {
    ...
    promptStyle:
      "soft lighting, before-after transformation, professional results, skincare routine, makeup application",
    storyboardTemplate: [
      {
        scene: 1,
        duration: 2,
        type: "hook",
        description: "Before state / problem",
      },
      {
        scene: 2,
        duration: 5,
  ...
  services: {
    ...
    promptStyle:
      "professional, trust-building, process showcase, testimonial style, expert showcase",
    storyboardTemplate: [
      ...
      },
      {
        ...
        duration: 4,
        type: "proof",
        description: "Results/testimonials",
      },
...
async function generateStoryboardWithLLM(
  ...
  try {
    ...
    logger.warn(
      `[VideoStoryboardService] generateStoryboardWithLLM parse failed: ${(err as Error).message}`,
    );
    return null;
  }
}
...
export class VideoStoryboardService {
  ...
  static generatePrompt(params: {
    ...
    const nicheTemplate =
      NICHE_TEMPLATES[params.niche as keyof typeof NICHE_TEMPLATES];
    const platformSpec =
      PLATFORM_SPECS[params.platform as keyof typeof PLATFORM_SPECS];
...
Visual style: Cinematic, high quality, engaging transitions.
  ...
  static async generateStoryboard(params: {
    ...
    const nicheTemplate =
      NICHE_TEMPLATES[params.niche as keyof typeof NICHE_TEMPLATES];
    const baseScenes =
      params.customScenes || nicheTemplate?.storyboardTemplate || [];
    ...
    try {
      ...
      if (storyboardConfig.provider !== "builtin") {
        const llmScenes = await generateStoryboardWithLLM(
          params,
          storyboardConfig,
        );
        if (llmScenes) {
      ...
      logger.warn(
        `[VideoStoryboardService] Storyboard LLM failed, using template: ${(err as Error).message}`,
      );
    }
  ...
  static async generateCaption(params: {
    ...
    if (lang === "id") {
      ...
      const baseCaption =
        nicheCaptions[Math.floor(Math.random() * nicheCaptions.length)];
    ...
    try {
      ...
      logger.warn(
        `Gemini caption generation failed for lang=${lang}: ${(err as Error).message}, falling back to English`,
      );
  ...
  static updateStoryboardTemplate(
    ...
    if (NICHE_TEMPLATES[niche as keyof typeof NICHE_TEMPLATES]) {
      ...
      (
        NICHE_TEMPLATES[niche as keyof typeof NICHE_TEMPLATES] as any
      ).storyboardTemplate = scenes;
    }
    return true;
  }
}
