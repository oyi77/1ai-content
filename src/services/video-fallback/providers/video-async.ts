...
async function pollUntilComplete(
  ...
  throw new ProviderTimeoutError(
    providerName,
    config.maxAttempts * config.intervalMs,
  );
}
...
async function generateViaFalai(
  ...
  if (!requestId || !statusUrl || !responseUrl) {
    throw new ProviderError(
      "Fal.ai",
      `incomplete queue response: ${JSON.stringify(submitRes.data)}`,
    );
  }
  ...
  const videoUrl = await pollUntilComplete("Fal.ai", requestId, async () => {
    ...
    if (status === "FAILED")
      throw new ProviderError(
        "Fal.ai",
        statusRes.data?.error || "generation failed",
      );
    if (status === "COMPLETED") {
      ...
      if (!url)
        throw new ProviderError(
          "Fal.ai",
          "COMPLETED but no video URL in result",
        );
  ...
  const videoUrl = await pollUntilComplete(
    ...
    async (id) => {
      ...
      if (status === "Failed")
        throw new ProviderError(
          "SiliconFlow",
          poll.data?.reason || "generation failed",
        );
...
async function generateViaXAI(
  ...
  const videoUrl = await pollUntilComplete("XAI", uuid, async (id) => {
    ...
    const { status: s, generated_video, error_message } = poll.data;
    if (s === 3)
      throw new ProviderError("XAI", error_message || "generation failed");
    if (s === 2 && generated_video?.length > 0) {
      return {
        status: "completed",
        videoUrl: generated_video[0].video_url || generated_video[0].video_uri,
...
async function generateViaEvoLink(
  ...
  if (
    ...
    throw new ProviderError(
      "EvoLink",
      `credits/access denied: ${errorMsg}`,
      403,
    );
  }
  ...
  const videoUrl = await pollUntilComplete("EvoLink", taskId, async (id) => {
    ...
    if (status === "failed" || status === "error") {
      throw new ProviderError(
        "EvoLink",
        `task failed: ${poll.data?.error || "Unknown error"}`,
      );
    }
    if (status === "completed") {
      let url = "";
      ...
      if (!url)
        throw new ProviderError("EvoLink", "completed but no video URL");
      return { status: "completed", videoUrl: url };
    }
    return { status: "pending" };
  });
...
async function generateViaHypereal(
  ...
  if (!jobId)
    throw new ProviderError(
      "Hypereal",
      `no jobId in response: ${JSON.stringify(response.data)}`,
    );
  ...
  const videoUrl = await pollUntilComplete("Hypereal", jobId, async (id) => {
    ...
    if (status === "failed")
      throw new ProviderError(
        "Hypereal",
        `job failed: ${poll.data?.error || "Unknown error"}`,
      );
    if (status === "completed") {
      const url = poll.data?.outputUrl || poll.data?.output_url || "";
      if (!url)
        throw new ProviderError("Hypereal", "completed but no video URL");
      return { status: "completed", videoUrl: url };
    }
    return { status: "pending" };
  });
...
async function generateViaKie(
  ...
  if (!taskId)
    throw new ProviderError(
      "Kie.ai",
      `no taskId in response: ${JSON.stringify(response.data)}`,
    );
  ...
  const videoUrl = await pollUntilComplete("Kie.ai", taskId, async (id) => {
    ...
    if (status === "failed" || status === "error" || status === "fail") {
      throw new ProviderError(
        "Kie.ai",
        `task failed: ${data.msg || "Unknown error"}`,
      );
    }
...
async function generateViaPiAPI(
  ...
  if (!taskId)
    throw new ProviderError(
      "PiAPI",
      `video: no task_id: ${JSON.stringify(response.data).slice(0, 200)}`,
    );
  ...
  const videoUrl = await pollUntilComplete("PiAPI", taskId, async (id) => {
    ...
    if (status === "failed")
      throw new ProviderError(
        "PiAPI",
        `video: ${pollData?.error || "task failed"}`,
      );
    if (status === "completed") {
      ...
      if (!url)
        throw new ProviderError("PiAPI", "video: completed but no video URL");
      return { status: "completed", videoUrl: url };
    }
    return { status: "pending" };
  });
...
async function generateViaRunware(
  ...
  const videoUrl = await pollUntilComplete("Runware", taskId, async (id) => {
    ...
    if (status === "failed")
      throw new ProviderError("Runware", "generation failed");
    return { status: "pending" };
  });
...
function agnesDimensions(aspectRatio?: string): {
  width: number;
  height: number;
} {
...
function agnesFrames(
  ...
  const n = Math.min(
    ...
    Math.max(
      1,
      Math.round(
        (Math.max(1, Math.round((duration ?? 5) * frameRate)) - 1) / 8,
      ),
    ),
  );
...
async function generateViaAgnes(
  params: VideoFallbackParams,
): Promise<VideoFallbackResult> {
  ...
  if (keys.length === 0)
    return {
      success: false,
      error: "AGNES_API_KEYS not configured",
      provider: "agnes_video",
    };

  const API_KEY = keys[agnesKeyIndex++ % keys.length];
  const base = (
    config.AGNES_API_BASE || "https://apihub.agnes-ai.com/v1"
  ).replace(/\/+$/, "");
  ...
  const resp = await axios.post(
    `${base}/videos`,
    {
      model,
      prompt: params.prompt,
      ...agnesDimensions(params.aspectRatio),
      ...agnesFrames(params.duration),
    },
    {
      headers: {
        Authorization: `Bearer ${API_KEY}`,
  ...
  const syncUrl =
    resp.data?.video_url ||
    resp.data?.url ||
    resp.data?.metadata?.url ||
    resp.data?.output?.url ||
    resp.data?.video?.url;
  if (syncUrl)
    return { success: true, videoUrl: syncUrl, provider: "agnes_video" };
  ...
  const videoUrl = await pollUntilComplete(
    ...
    async (id) => {
      ...
      if (status === "failed" || status === "error" || status === "expired")
        throw new ProviderError("Agnes", "generation failed");
      return { status: "pending" };
...
async function generateViaOmniRouteVideo(
  ...
  const videoUrl = await pollUntilComplete("OmniRoute", taskId, async (id) => {
    ...
    if (status === "failed")
      throw new ProviderError("OmniRoute", "generation failed");
    return { status: "pending" };
  });
  return { success: true, videoUrl, provider: "omniroute" };
}

async function generateViaMPT(
  params: VideoFallbackParams,
): Promise<VideoFallbackResult> {
  const config = getConfig();
  const PEXELS_API_KEY =
    config.PEXELS_API_KEYS?.split(",")?.[0] || config.PEXELS_API_KEY || "";
