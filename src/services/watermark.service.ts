...
Respond with ONLY the JSON, no other text.`;
      ...
      if (usageMeta) {
        trackTokens({
          ...
          model,
          service: "watermark_detection",
          promptTokens: usageMeta.promptTokenCount || 0,
          completionTokens: usageMeta.candidatesTokenCount || 0,
        }).catch((err) =>
          logger.warn("Token tracking failed", {
      ...
      logger.info(
        `🔍 Watermark detection: ${result.hasWatermark ? "FOUND" : "clean"} (confidence: ${result.confidence})`,
      );
      return {
  ...
  static async removeFromImage(
    ...
    const filters = regions.map((r) => {
      ...
      const w = Math.min(
        imgW - x,
        Math.max(10, Math.floor((r.width / 100) * imgW)),
      );
      const h = Math.min(
        imgH - y,
        Math.max(10, Math.floor((r.height / 100) * imgH)),
      );
  ...
  static async removeFromVideo(
    ...
    const filters = regions.map((r) => {
      ...
      const w = Math.min(
        vidW - x,
        Math.max(10, Math.floor((r.width / 100) * vidW)),
      );
      const h = Math.min(
        vidH - y,
        Math.max(10, Math.floor((r.height / 100) * vidH)),
      );
      return `delogo=x=${x}:y=${y}:w=${w}:h=${h}`;
    });
    ...
    await execFile(
      "ffmpeg",
      ["-y", "-i", inputPath, "-vf", filterStr, "-c:a", "copy", outputPath],
      { timeout: 120000 },
    );
  ...
  static async cleanImage(imageUrl: string): Promise<string | null> {
    try {
      ...
      const cleanedPath = await this.removeFromImage(
        tmpFile,
        detection.regions,
      );
  ...
  static async cleanVideo(videoPath: string): Promise<string> {
    try {
      ...
      logger.info(
        `🔍 Watermark detected in video, removing ${detection.regions.length} region(s)...`,
      );
   ...
   */
  private static async detectFromBase64(
    base64: string,
    mimeType: string,
    ...
    try {
      ...
      if (usageMeta2) {
        trackTokens({
          provider: "gemini-direct",
          model,
          service: "watermark_clean",
          promptTokens: usageMeta2.promptTokenCount || 0,
          completionTokens: usageMeta2.candidatesTokenCount || 0,
        }).catch((err) =>
          logger.warn("Token tracking failed", {
            error: (err as Error).message,
          }),
