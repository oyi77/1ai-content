...
const TOOLS = [
  {
    ...
    description:
      "Check health status of 1ai-content services (video, image, ebook, social)",
  ...
  },
  {
    ...
    description:
      "Generate an AI video. Available niches: fnb, fashion, tech, health, travel, education, finance, entertainment",
    inputSchema: {
      type: "object",
      properties: {
        telegramId: { type: "string", description: "User's Telegram ID" },
        ...
        customPrompt: {
          type: "string",
          description: "Custom prompt for video content",
        },
        platform: {
          type: "string",
          description: "Target platform: tiktok, instagram, youtube, square",
        },
      },
  ...
  },
  {
    ...
    description:
      "Check status of a video generation job. When a telegramId is supplied, the job is scoped to that user (their job ID only).",
    inputSchema: {
      type: "object",
      properties: {
        ...
        telegramId: {
          type: "string",
          description:
            "User's Telegram ID (optional, scopes lookup to that user)",
        },
      },
      required: ["jobId"],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        category: {
          type: "string",
          description: "Image category: product, fnb, realestate, car, general",
        },
        aspectRatio: {
          type: "string",
          description: "Aspect ratio: 9:16, 1:1, 16:9, 4:5",
        },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        idea: {
          type: "string",
          description: "Ebook idea or topic (10-5000 chars)",
        },
        title: {
          type: "string",
          description: "Ebook title (optional, auto-generated if empty)",
        },
        chapterCount: {
          type: "number",
        ...
        targetLanguage: {
          type: "string",
          description: "Language code: id, en, ms, th, vi, tl",
        },
        productMode: {
          type: "string",
          description:
            "Product mode: lead_magnet, paid_ebook, bonus, authority",
        },
        telegramId: {
          type: "string",
          description:
            "Owner's Telegram ID — scopes the project to this user. Omit to create an unscoped project.",
        },
      },
      required: ["idea"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        telegramId: {
          type: "string",
          description:
            "Owner's Telegram ID — required to read another user's scoped project; without it only unscoped (legacy) projects resolve",
        },
      },
      required: ["projectId"],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        limit: {
          type: "number",
          description: "Max ebooks to return (default 10)",
        },
        telegramId: {
          type: "string",
          description:
            "Owner's Telegram ID — returns only this user's scoped projects; omit for legacy (unscoped) projects",
        },
      },
      required: [],
    },
    ...
    try {
      switch (name) {
        ...
        case "1ai-content_list_videos": {
          const { telegramId, limit = 20 } = args as {
            telegramId: string;
            limit?: number;
          };
        ...
        case "1ai-content_create_video": {
          ...
          const nicheConfig = (NICHES as Record<string, { styles?: string[] }>)[
            niche
          ];
          if (!nicheConfig) {
            return {
              content: [
                {
                  type: "text",
                  text: `Invalid niche. Available: ${Object.keys(NICHES).join(", ")}`,
                },
              ],
              isError: true,
            };
          ...
          await enqueueVideoGeneration({
            ...
            storyboard: storyboard.map((s, i) => ({
              scene: i + 1,
              duration: s.duration || Math.floor(duration / scenes),
        ...
        case "1ai-content_get_video_status": {
          const { jobId, telegramId } = args as {
            jobId: string;
            telegramId?: string;
          };
          const video = await prisma.video.findFirst({
            where: telegramId
              ? { jobId, userId: BigInt(telegramId) }
              : { jobId },
            select: {
              jobId: true,
              status: true,
                ...
                text: JSON.stringify(
                  {
                    ...
                    status: "generating",
                    message:
                      "Ebook generation started. Use 1ai-content_get_ebook_status to check progress.",
                  },
                  null,
                  2,
                ),
        ...
        case "1ai-content_get_ebook_status": {
          const { telegramId, projectId } = args as {
            telegramId?: string;
            projectId: number;
          };
        ...
        case "1ai-content_list_ebooks": {
          const { telegramId, limit = 10 } = args as {
            telegramId?: string;
            limit?: number;
          };
          ...
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify({ ebooks: projects }, null, 2),
              },
            ],
          };
        }
        ...
        case "1ai-content_ai_chat": {
          const { telegramId, message } = args as {
            telegramId: string;
            message: string;
          };
              ...
              content: [
                {
                  type: "text",
                  text: JSON.stringify(
                    { response: response.content, model: response.model },
                    null,
                    2,
                  ),
                },
          ...
          return {
            content: [
              {
                type: "text",
                text: response.error || "Chat generation failed",
              },
            ],
            isError: true,
          };
        ...
        case "1ai-content_get_niches": {
          const niches = Object.entries(
            NICHES as Record<string, { label?: string; styles?: string[] }>,
          ).map(([key, config]) => ({
            key,
          ...
          return {
            content: [
              { type: "text", text: JSON.stringify({ niches }, null, 2) },
            ],
          };
        }
