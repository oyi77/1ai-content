...
- `analyzer.py` — `TrendAnalyzer` l.18, `analyze_trends` l.26, `_call_llm` l.135 (model `auto/all-working`), CLI l.156.
- `seo_generator.py` — `SEOGenerator` l.74; `PLATFORM_PRESETS` l.20 (tiktok/instagram/youtube/facebook/x/linkedin/threads).
  ...
  - `_call_llm` l.151 (httpx POST `{OMNIRoute_URL}/chat/completions`, model `auto/all-working`, timeout 60); `_build_prompt` l.172 + 7 `_prompt_*` l.197-374; `_parse_json` l.379 (regex `\{[\s\S]*\}`); CLI l.391; URL OmniRoute dari env `OMNIRoute_URL` l.17.
