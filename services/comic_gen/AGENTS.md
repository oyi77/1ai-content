...
- `panel_gen.py`: env `OMNIROUTE_BASE` (24, default localhost:8000), `OMNIROUTE_IMAGE_MODEL` (25, default `"black-forest-labs/flux-1-dev"`); `_try_omniroute` (30); `AGENTCASH_BASE="https://stablestudio.dev"` (63), `AGENTCASH_MODEL` (64, default `"stable-diffusion-3.5-large"`); `_try_agentcash` (67, poll 30×2s); `_shape_dimensions` (129); `_build_image_prompt` (148); `_get_font` (156); `_render_placeholder` (198); `generate_panel_image` (239, chain OmniRoute→AgentCash→Placeholder, selalu return PIL).
...
## Issue Spesifik
- [MEDIUM] `panel_gen.py:101-102` `_try_agentcash`: `image_url` bisa `UnboundLocalError` jika loop `while` keluar karena `status != 200`; error ini tertelan oleh except umum (119) → kegagalan gambar berubah jadi fallback placeholder yang tidak dicatat (silent fail).
