"""ClipperContentGenerator — ContentGenerator wrapper for the Clipper engine.
...
generator); the engine is acquired lazily from ``services.di.get_clipper()`` so
constructing this generator stays cheap (api.py builds it at import time).

The Clipper engine itself stays untouched for direct callers: the thin
``/clipper/clip`` router passes no hooks and behaves exactly as before.
"""
...
class ClipperContentGenerator(ContentGenerator):
    """In-memory project lifecycle around ``ClipperEngine.clip_video``."""
    ...
    def info(self) -> GeneratorInfo:
        return GeneratorInfo(
            name="clipper",
            description="Video clipping: source → transcribe → highlights → per-clip processing",
            version="1.0",
            capabilities=["clipping"],
        )

    async def create(self, params: dict, *, owner: str | None = None) -> dict:
        try:
            ClipperClipRequest.model_validate(params)
