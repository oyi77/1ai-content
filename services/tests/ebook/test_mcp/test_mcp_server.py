...
import services.ebook.mcp.server as server
...
def test_get_status_returns_db_status_for_known_project():
    ...
    with (
        patch("services.ebook.mcp.server._get_repo", return_value=repo),
...
def test_list_files_returns_file_list(tmp_path):
    ...
    with patch("services.ebook.mcp.server.PROJECTS_DIR", tmp_path):
