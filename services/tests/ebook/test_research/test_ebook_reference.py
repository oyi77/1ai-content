...
from services.ebook.research.ebook_reference import (
    EbookRef,
...
class TestSearchGoogleBooks:
    ...
    def test_description_truncated_at_500(self, mock_urlopen):
        ...
        data = {
            "items": [
                {"volumeInfo": {"title": "Long Desc Book", "description": long_desc}}
            ]
        }
...
class TestSearchEbooks:
    ...
    @patch(
        "services.ebook.research.ebook_reference._search_google_books",
        side_effect=Exception("network"),
    )
    @patch("urllib.request.urlopen")
    def test_falls_back_to_open_library_on_google_failure(
        self, mock_urlopen, _mock_google
    ):
        mock_urlopen.return_value = _make_mock_urlopen(OPEN_LIBRARY_RESPONSE)
    ...
    @patch(
        "services.ebook.research.ebook_reference._search_google_books", side_effect=Exception("g")
    )
    @patch(
        "services.ebook.research.ebook_reference._search_open_library", side_effect=Exception("ol")
    )
    ...
    def test_deduplicates_across_sources(self, mock_urlopen):
        ...
        google_data = {
            "items": [{"volumeInfo": {"title": "Deep Work", "language": "en"}}]
        }
        ol_data = {
            "docs": [
    ...
    def test_max_results_capped_at_40(self):
        with patch(
            "services.ebook.research.ebook_reference._search_google_books", return_value=[]
        ) as _g:
            with patch(
                "services.ebook.research.ebook_reference._search_open_library", return_value=[]
            ) as _ol:
