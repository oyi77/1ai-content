...
| `health-check.sh` | Disk/mem threshold 80%, cek proses node/postgres/redis-server, port 3000/5432/6379, HTTP `/health /health/db /health/queue`; eksternal hanya jika `CHECK_EXTERNAL` (GEMINIGEN_API_URL, ping Midtrans) |
...
| `systemd/1ai-content-bot.service` | `npx tsx src/index.ts`, User=openclaw, EnvironmentFile=.env, MemoryMax=1G, CPUQuota=80%, Restart=always, After=redis+postgresql |
