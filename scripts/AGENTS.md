...
> Last updated: 2026-08-02 — onboarding + DAILY REPORT DIHAPUS (worker `src/workers/daily-report.worker.ts` + script `scripts/daily-report.ts` — chat 6077091585 tak reachable → 400 `chat not found`); e2e playwright diisolasi :3002 → :3111 + DB test `1ai_content_test`

# AGENTS.md — scripts

## Tujuan Folder Ini

Script DevOps & operasional: setup lingkungan, deploy, backup, health-check, dan pipeline konten TikTok/IG. Script bash/Node/Python berdiri sendiri — pastikan executable (`chmod +x`).
