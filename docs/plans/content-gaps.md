...
- Files:
  ...
  - `services/routers/subtitles.py` — router `APIRouter(prefix="/video")`, `POST /video/subtitles`. (historical note superseded 2026-08-05: legacy shims `services/routers/{compat,content}.py` carrying `/captions/*` and `/text/caption*` were migrated to modern equivalents and DELETED; those legacy paths now return 404.)
